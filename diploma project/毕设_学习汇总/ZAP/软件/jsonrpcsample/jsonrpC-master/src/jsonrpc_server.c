...
//**所有的操作，都是直接对全局结构体变量进行操作，而不需要返回特定的分步操作结果**//
	...
	error = self->net.error(self->net_handle);		//**error接收数据，并将接受到的数据进行解析，存储在rcvmsg；同时负责下一次循环回来时，对上一次包装好的数据的发送**//
	if (error != JSONRPC_ERROR_OK)					//**error = recv、parse、send(next turn)**//
		return error;

	req = self->net.recv(self->net_handle, timeout, &desc);		//**修改解析状态位，导出具体json请求**//
	if (req == NULL)
	...
	return self->net.send(self->net_handle, res, desc);			//**对调用结果进行包装，等待被发送给客户端**//
}
...
jsonrpc_error_t	jsonrpc_notify_client (
				...
				, ...)
{
	static unsigned short global_sequence = 0;
	size_t  size;
	int		ret;
	int     i;
	jsonrpc_mstream_t *stream;
		...
		if (ret <= (int)self->param.argc)/*缓冲区够用*/
		{
			...
			jsonrpc_mstream_print(stream, "{");
			{
				...
				if (ret > 0)
				{
					...
					for(i=0;i<ret;i++)
					{
						if(i!=0) jsonrpc_mstream_print(stream, ",");
						...
						case JSONRPC_TYPE_STRING:
							...
							jsonrpc_mstream_print(stream, "%s", escaped_string(valuestring));
							free(escapedstring);
							break;
						case JSONRPC_TYPE_ARRAY:
							break;
				...
				jsonrpc_mstream_print(stream, ",\"id\":%d", ++global_sequence);
    ...
    self->net.send(self->net_handle, res, NULL);
	printf("notify : %s\n", res);
	return JSONRPC_ERROR_OK;
}
