...
#if 0
#define MAXLINE 2000 
boolean socket_start_flag = FALSE_C;
int serv_sock = -1;
...
void acceptThread(){
    ...
    while(1){
      sleep(1);
      int n1 = recv(clnt_sock, buff, MAXLINE, 0);
      if(n1==0){
        continue;
      }
      n=n1;
 ...
 * records.  The application should return TRUE_C if the message was processed or
 * FALSE_C if it was not.
 *
 * @param cmd   Ver.: always
 */
 ...
 * application should return TRUE_C if the message was processed or FALSE_C if it
 * was not.
 *
 ...
 * TRUE_C if the message was processed or FALSE_C if it was not.
 *
 * @param clusterId The cluster identifier of this response.  Ver.: always
 ...
 * discovered.  TRUE_C if there are no more attributes to be discovered.  Ver.:
 * always
 ...
 * endpoint is a member of a group.  The application should return TRUE_C if the
 * endpoint is a member of the group and FALSE_C otherwise.
 *
 * @param endpoint The endpoint.  Ver.: always
 ...
 * If the index is overridden than the callback shall return TRUE_C and set the
 * endpointReturn parameter accordingly. A value of 0xFF means the endpoint
 * doesn't exist at that index.
        Otherwise FALSE_C must be returned by the
 ...
 * overridden then the callback must return TRUE_C.  Otherwise it should return
 * FALSE_C, which allows the framework to perform its default behavior.  This is
 ...
 * provide data then TRUE_C shall be returned.  Otherwise the callback must return
 * FALSE_C.
 *
 ...
 * disallow the operation.  If the application returns TRUE_C then the key
 * establishment is allowed to proceed.  If it returns FALSE_C, then key
 ...
 * callback should return a boolean value of TRUE_C or FALSE_C. A value of TRUE_C
 ...
 * will be TRUE_C and the application should only issue EZSP commands that affect
 * memory allocation on the NCP.  For example, tables on the NCP can be resized
 * in the first call.  In the second invocation, memoryAllocation will be FALSE_C
                                                 ...
                                                 const EmberAfOtaImageId* id)
{
  ...
  if (!success) {
    ...
    return TRUE_C;   // return value is ignored
  }
 ...
 * @param returnValue If the server returns TRUE_C indicating that the client
                                                  ...
                                                  const EmberAfOtaImageId* imageId)
{
  ...
  // return FALSE_C.
  // If the server wants the client to apply the upgrade, it should return TRUE_C
 ...
 * partner link key exchange.  The application should return TRUE_C to accept the
 * request or FALSE_C to reject it.  On network coprocessor platforms, this
 ...
 * performing key establishment.  The application should return TRUE_C if key
 ...
 *  If the application returns TRUE_C, the plugin will attempt to join the
 ...
 * returns TRUE_C it is assumed that the callback has consumed and processed the
 * message. The framework will not do any further processing on the message.
   
 *     If the function returns FALSE_C then it is assumed that the callback has
 ...
 * has been handled. If the callback returns a value of TRUE_C, then the
 * Application Framework assumes that the message has been handled and it does
 * nothing else with it. If the callback returns a value of FALSE_C, then the
 ...
 * ZDO, or some other protocol.  This is called prior to
        any ZigBee
 * fragmentation that may be done.  If the function returns TRUE_C it is assumed
 ...
 * framework will do no further processing on the message.
        If the
 * function returns FALSE_C then it is assumed that the callback has not processed
 ...
 * FALSE_C indicating that the incoming ZDO message has not been handled and
 ...
 * return TRUE_C if the message was processed or FALSE_C if it was not.
 *
 * @param clusterId The cluster identifier of this response.  Ver.: always
 ...
 * should return TRUE_C if the message was processed or FALSE_C if it was not.
 *
 * @param cmd   Ver.: always
 */
 ...
 * registration is complete. This callback provides a success value of TRUE_C if
 * the registration process was successful and a value of FALSE_C if registration
 * failed.
 *
 * @param success TRUE_C if registration succeeded, FALSE_C otherwise.  Ver.: always
 */
                                    ...
                                    EmberJoinDecision decision)
{
  ...
  if(status==EMBER_STANDARD_SECURITY_UNSECURED_JOIN){
     newNodeNet(newNodeId,newNodeEui64);
  }else if(status==EMBER_DEVICE_LEFT){
     nodeLeftNet(newNodeId,newNodeEui64);
  }
