...
*/
typedef enum {
  ASH_SERIAL_PORT_CLOSED = 0,
  ASH_SERIAL_PORT_OPENED = 1,
} AshSerialPortEvent;

/** @brief Register a callback that will get called whenever the ASH serial
 *  file descriptor is opened or closed.
 */
typedef void (*AshSerialPortCallbackFunction)(AshSerialPortEvent event, int fileDescriptor);
