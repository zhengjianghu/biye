...
static AshSerialPortCallbackFunction ashSerialPortCallbackFunction = NULL;
...
static EzspStatus ashInternalSerialInit(boolean ignoreErrors)
{
  ...
  if (EZSP_SUCCESS == ashSetupSerialPort(&serialFd,
    ...
    if (NULL != ashSerialPortCallbackFunction) {
      ashSerialPortCallbackFunction(ASH_SERIAL_PORT_OPENED, serialFd);
    }
    return EZSP_SUCCESS;
  }
...
void ashSerialClose(void)
{
  if (serialFd != NULL_FILE_DESCRIPTOR) {
    ...
    if (NULL != ashSerialPortCallbackFunction) {
      ashSerialPortCallbackFunction(ASH_SERIAL_PORT_CLOSED, serialFd);
    }
...
boolean ashSerialPortRegisterCallback(AshSerialPortCallbackFunction callback)
{
  if (ashSerialPortCallbackFunction != NULL) {
    return FALSE;
  }
  ashSerialPortCallbackFunction = callback;
  return TRUE;
}
