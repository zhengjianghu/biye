...
static void * ashTask = NULL;
void ashTick(void * para);
void * halStartPeriodTask(void(* task)(void *), void * para, int interval);
void halStopPeriodTask(void * task);
EzspStatus ashStart(void)
{
  ...
  ashTask = halStartPeriodTask(ashTick, NULL, 100);
  if(ashTask == NULL)
  {
	  ashStopAckTimer();
...
void ashTick(void * para)
{
	static int8u stay_in_recv = 0;
  EzspStatus status;
  if ((!(ashFlags & FLG_CONNECTED))||stay_in_recv) {
	  if(stay_in_recv == 0)
	  {
		  ashSendExec();
	  }
        ...
        if(status == EZSP_ASH_IN_PROGRESS)
        {
        	stay_in_recv = 1;
        	return;
        }
