                                          ...
                                          int8u stopBits)
{
  ...
  if (port==COM_USART0)
  {
    // ignoring baudrate/parity/stopbits for now
    COM_Init_t initdata = COM_INITUART(USART0_INIT);
  ...
  {
    // ignoring baudrate/parity/stopbits for now
    COM_Init_t initdata = COM_INITUART(USART1_INIT);
    status = COM_Init((COM_Port_t) port, &initdata);
  }
#endif
  ...
  {
    // ignoring baudrate/parity/stopbits for now
    COM_Init_t initdata = COM_INITUART(USART2_INIT);
    status = COM_Init((COM_Port_t) port, &initdata);
  }
#endif
  return status;
...
int16u emberSerialWriteUsed(int8u port)
{
...
boolean emberSerialUnused(int8u port)
{
#ifdef EMBER_SERIAL_USE_STDIO
  return FALSE;
