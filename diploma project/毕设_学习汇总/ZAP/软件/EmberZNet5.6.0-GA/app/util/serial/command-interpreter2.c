...
void emberCommandReaderSetDefaultBase(int8u base)
{
  defaultBase = base;
}
...
boolean emberStringToHostOrderIpv4Address(const int8u* string, int32u* hostOrderIpv4Address)
{
  int8u quadCount = 0;
  int16u quad = 0;  // 16-bit so we can catch errors where quad > 255.
  ...
  *hostOrderIpv4Address = 0;
  while (*string != '\0' && *string != '"') {
    if (*string == '.') {
      ...
      *hostOrderIpv4Address += (int8u)quad;
...
boolean emberStringArgumentToHostOrderIpv4Address(int8u argNum, int32u* hostOrderIpv4Address)
{
  int8u argLength;
  int8u *contents = emberStringCommandArgument(argNum,
                                               &argLength);
...
int8u emberCopyBigEndianEui64Argument(int8s index, EmberEUI64 destination)
{
  // emberReverseMemCopy may not be available, so we have to do it ourselves.
  int8u i, length = emberCopyEui64Argument(index, destination);
  for (i = 0; i < EUI64_SIZE / 2; i++) {
    int8u tmp = destination[EUI64_SIZE - i - 1];
