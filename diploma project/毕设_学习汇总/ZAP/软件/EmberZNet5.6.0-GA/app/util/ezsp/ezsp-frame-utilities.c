...
int8u *fetchInt8uPointer(int8u length)
{
...
void emAfFetchOrAppend(boolean fetch, PGM_P format, ...)
{
  ...
  while (*c) {
    ...
    switch (*c) {
    case 's': {
      int8s *realPointer = (int8s *)ptr;
      if (fetch) {
        *realPointer = (int8s)fetchInt8u();
      } else {
        appendInt8u((int8u)*realPointer);
      }
    }
      break;
    case 'u': {
      int8u *realPointer = (int8u *)ptr;
      if (fetch) {
    ...
    case 'v': {
      ...
      break;
    case 'w': {
      int32u *realPointer = (int32u *)ptr;
      if (fetch) {
        *realPointer = fetchInt32u();
      } else {
        appendInt32u((int32u)*realPointer);
      }
    }
      break;
    case 'b': {
      int8u *realArray = (int8u *)ptr;
      int8u *lengthPointer = (int8u *)va_arg(args, PointerType);
      if (fetch) {
