...
//FIXME: No ADC API exists yet on EFR32
#define halStartAdcConversion(a,b,c,d) /* no-op */
#define halReqeustAdcData(a,pData) (*(pData) = halCommonGetRandom(), \
                                    EMBER_ADC_CONVERSION_DONE)
#define halReadAdcBlocking(a,pData) halReqeustAdcData(a,pData)
...
#ifdef  CORTEXM3_EFR32
int8u dataMode = DATA_MODE_RANDOM; 
#else//!CORTEXM3_EFR32
...
void sendData(void) {
  ...
  MEMMOVE(&(globalBuffer[0]), emberGetEui64(), EUI64_SIZE);
...
void sendMulticastHello(void) {
  ...
  MEMMOVE(&(globalBuffer[0]), emberGetEui64(), EUI64_SIZE);
  MEMMOVE(&(globalBuffer[EUI64_SIZE]), data, HELLO_MSG_SIZE);
...
void handleSinkAdvertise(int8u* data) {
  ...
  MEMMOVE(sinkEUI, &(data[0]), EUI64_SIZE);
...
void printHelp(void)
{
  ...
  PRINT("n = print internal counter data\r\n");
