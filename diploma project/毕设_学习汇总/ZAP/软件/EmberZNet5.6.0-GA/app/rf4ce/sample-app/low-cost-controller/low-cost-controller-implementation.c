...
// Prototypes
static void keyPressedHandler(int8u key);
static void keyReleasedHandler(int8u key);
static void decodeActionForKey(int8u key, TxConfig *txConfig);

// This table keeps track of what each device (TV or STB) is paired to.
static int8u pairingDeviceTable[DEVICE_COUNT] = {
  NO_PAIRING,
  NO_PAIRING
};

static const int8u deviceType[DEVICE_COUNT] = {
  EMBER_AF_RF4CE_DEVICE_TYPE_SET_TOP_BOX,
  EMBER_AF_RF4CE_DEVICE_TYPE_TELEVISION
};

// State of keys during last scan
static boolean lastKeyStates[KEY_MATRIX_NUM_KEYS]; // Initialized to 0s
// Which device mode is active (STB or TV). Device mode is changed with TV
// or STB keys
static int8u   activeDevice          = DEVICE_STB;

static boolean anyKeyDown            = FALSE;
static boolean bindingActive         = FALSE;
static boolean pairingKeyPressed     = FALSE;
static boolean stbKeyPressed         = FALSE;
static boolean tvKeyPressed          = FALSE;
...
void emberAfMainInitCallback(void)
{
  ...
  halCommonGetIndexedToken(&pairingDeviceTable[DEVICE_STB],
                           TOKEN_PAIRING_DEVICE_TABLE,
                           DEVICE_STB);

  halCommonGetIndexedToken(&pairingDeviceTable[DEVICE_TV],
                           TOKEN_PAIRING_DEVICE_TABLE,
                           DEVICE_TV);
...
static void keyPressedHandler(int8u key)
{
  ...
  TxConfig txConfig;
  int8u keyRfCode = defaultActionsRf[key].code;
...
static void keyReleasedHandler(int8u key)
{
  ...
  if (key == DBG_KEY_PAIR) {
    ...
    // Stop RF if not atomic
    if (txConfig.rfSpecified && !txConfig.atomic) {
...
static void decodeActionForKey(int8u key, TxConfig *txConfig)
{
  ...
  boolean useMappedAction;
  int8u keyRfCode = defaultActionsRf[key].code;
  int8u keyRfConf = defaultActionsRf[key].config;
  ...
  if (useMappedAction) {
    ...
    if(activeDevice == DEVICE_TV) {
      ...
      txConfig->irCode         = (int8u *)defaultActionsIr[key].data;
    } else {
