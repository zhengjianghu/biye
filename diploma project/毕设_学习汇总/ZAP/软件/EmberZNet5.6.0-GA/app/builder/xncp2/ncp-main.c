...
static void createEndpoint(int8u endpointIndex);
static int8u ezspNextSequence(void);

//------------------------------------------------------------------------------
// Functions

boolean emberAfMemoryByteCompare(const int8u* pointer, int8u count, int8u byteValue)
{
  int8u i;
...
void emAfResetAndInitNCP(void)
{
  ...
  halStackSeedRandom(((int32u)seed1 << 16) | (int32u)seed0);
                                     ...
                                     PGM_P configIdName)
{
  ...
  LogDebug("Ezsp Config: set %s to 0x%2x:", configIdName, value);
                                ...
                                PGM_P decisionName)
{
  ...
  LogDebug("Ezsp Policy: set %s to \"%s\":", policyName, decisionName);
...

{
  ...
  case 2:
  case 4:
    LogAppend(LOG_DEBUG, "0x%4x:", promotedValue);
    break;
  default:
    LogAppend(LOG_DEBUG, "{val of len %x}:", valueLength);
  }
...
void emAfCliVersionCommand(void)
{
  ...
  if (EZSP_SUCCESS != ezspGetVersionStruct(&versionStruct)) {
    // NCP has Old style version number
	  emberAfAppPrint("stack ver [0x%2x]\n", ncpStackVer);
  } else {
    // NCP has new style version number
      //todo emAfParseAndPrintVersion(versionStruct);
  }
  emberAfAppFlush();
}
