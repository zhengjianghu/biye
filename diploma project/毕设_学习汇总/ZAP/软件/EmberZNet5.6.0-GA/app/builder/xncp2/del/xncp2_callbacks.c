...
static void sendLedProtocolCommand(int8u command, int32u possibleParam)
{
  EmberStatus status;
  int8u commandLength = 0;
  int8u commandPayload[LED_PROTOCOL_MAX_FRAME_LENGTH];
  int8u replyLength;
  int8u replyPayload[LED_PROTOCOL_MAX_FRAME_LENGTH];
  ...
  if (command == LED_PROTOCOL_COMMAND_GET_FREQ) {
    ...
    int8u ledState = replyPayload[LED_PROTOCOL_RESPONSE_INDEX];
...
{
  int32u frequency = (int32u)emberUnsignedCommandArgument(0);
...
{
  int8u command = (int8u)emberUnsignedCommandArgument(0);
