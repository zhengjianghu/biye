...
DEFINE_LOGGER(ASH, 128, LOG_INFO);
...
static AshHostConfig ashHostConfigArray[] =
{
  ...
  { // Config 1: EM2xx/EM3xx @ 57600 bps with XON/XOFF flow control
    ...
    256,                  // max bytes to buffer before writing to serial port
    256,                  // max bytes to read ahead from serial port
    255,                    // trace output control bit flags
    3,                    // max frames sent without being ACKed (1-7)
    TRUE,                 // enables randomizing DATA frame payloads
    800,                  // adaptive rec'd ACK timeout initial value
    400,                  //  "     "     "     "     "  minimum value
...
EzspStatus serialResponseReceived(void)
{
  ...
  while (buffer != NULL) {
    ...
    if (waitingForResponse
      ...
      ashTraceEzspVerbose("serialResponseReceived(): ID=0x%x Seq=0x%x Buffer=\n",
      ...
      ashTraceEzspVerbose("serialResponseReceived(): ashFreeBuffer(): \n");
      buffer = NULL;
      status = EZSP_SUCCESS;
      waitingForResponse = FALSE;
    }
  ...
  if (dropBuffer != NULL) {
    ...
    ashTraceEzspVerbose("serialResponseReceived(): ashFreeBuffer(): drop \n");
...
EzspStatus ashStart(void)
{
  ...
  // Host needs time to wait for USB to re-enumerate when resetting.
...
static EzspStatus ashReceiveFrame(void)
{
  ...
  case TYPE_DATA:
    ...
    if (frmNum == frmRx) {                    // is frame in sequence?
      ...
      ashTraceEzspBuffVerbose("ashReceiveFrame(): ", rxDataBuffer->data, rxDataBuffer->len);
  ...
  case TYPE_INVALID:                          // reject invalid frames
    ashTraceArray("Rec'd frame:", rxLen, rxBuffer);
    ashRejectFrame();
    break;
...
EzspStatus ashReceive(int8u *len, int8u *inbuf)
{
  ...
  *len = buffer->len;
  ashFreeBuffer(&rxFree, buffer);
  ashTraceEzspVerbose("ashReceive(): ashFreeBuffer(): \n");
  return EZSP_SUCCESS;
}
...
EzspStatus ashSend(int8u len, const int8u *inptr)
{
  ...
  buffer->len = len;
  ashTraceEzspBuffVerbose("ashSend(): ", buffer->data, buffer->len);
...
static void ashScrubReTxQueue(void)
{
  ...
  while (ackRx != frmReTxHead) {
    ...
    ashTraceEzspVerbose("ashScrubReTxQueue(): ashFreeBuffer(): \n");
    INC8(frmReTxHead);
  }
}
