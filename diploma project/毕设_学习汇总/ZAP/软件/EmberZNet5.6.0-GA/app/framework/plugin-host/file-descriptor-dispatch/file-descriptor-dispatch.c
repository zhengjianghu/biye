...
static boolean executingCallbacks = FALSE;
...
EmberStatus emberAfPluginFileDescriptorDispatchAdd(EmberAfFileDescriptorDispatchStruct* dispatchStruct)
{
  ...
  if (dispatchStruct->operation == EMBER_AF_FILE_DESCRIPTOR_OPERATION_NONE
      || dispatchStruct->operation > EMBER_AF_FILE_DESCRIPTOR_OPERATION_WRITE) {
...
EmberStatus emberAfPluginFileDescriptorDispatchWaitForEvents(int32u timeoutMs)
{
  ...
  struct timeval timeoutStruct = {
    ...
    0,                   // micro seconds (set later)
  };
  timeoutStruct.tv_usec = (timeoutMs % 1000000) * 1000;
  ...
  while (iterator != NULL) {
    if (iterator->dispatchStruct.callback != NULL) {
      if (!iterator->markedForRemoval
          && FD_ISSET(iterator->dispatchStruct.fileDescriptor, &readSet)) {
        (iterator->dispatchStruct.callback)(iterator->dispatchStruct.dataPassedToCallback,
                                            EMBER_AF_FILE_DESCRIPTOR_OPERATION_READ);
      }
      // NOTE:  We must check markedForRemoval before each callback since a previous callback
      // may have modified the state.
      if (!iterator->markedForRemoval
          && FD_ISSET(iterator->dispatchStruct.fileDescriptor, &writeSet)) {
        (iterator->dispatchStruct.callback)(iterator->dispatchStruct.dataPassedToCallback,
