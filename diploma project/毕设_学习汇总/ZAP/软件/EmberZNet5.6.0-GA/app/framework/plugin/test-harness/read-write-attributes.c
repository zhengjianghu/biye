...
static int8u sourceEndpoint = 1;
static int8u destEndpoint;
static int16u testClusterId;
static int16u attributeIdStart = 0;
static int16u attributeIdEnd = 0;
static boolean clientToServer = TRUE;

static int16u currentIndex;
...
static int16u timeoutSeconds = 3;
...
void setOptionsCommand(void)
{
  ...
  timeoutSeconds = (int16u)emberUnsignedCommandArgument(1);
  testType = temp;
}
...
void setDestinationCommand(void) 
{
  ...
  destEndpoint = (int8u)emberUnsignedCommandArgument(1);
}

static void readWriteAttributeTest(void)
{
  int16u count = attributeIdEnd - attributeIdStart + 1;
...
void startTestCommand(void)
{
  testClusterId = (int16u)emberUnsignedCommandArgument(0);
  attributeIdStart = (int16u)emberUnsignedCommandArgument(1);
  attributeIdEnd = (int16u)emberUnsignedCommandArgument(2);
  clientToServer = (boolean)emberUnsignedCommandArgument(3);
...
static const char* gettAttributeTypeString(int8u attributeType)
{
  int8u i = 0;
...
static const char* getErrorString(int8u status)
{
  int8u i = 0;
...
boolean checkTestComplete(void)
{
...
static void sendCommand(boolean read)
{
...
static void sendWriteAttribute(int8u type, 
	                           int8u* data, 
	                           int16u dataLen)
{
  emberAfFillExternalBuffer((ZCL_PROFILE_WIDE_COMMAND
                                                            ...
                                                            int16u bufLen)
{
  if (clusterId == testClusterId
  	  && readWriteState == READ_WRITE_TEST_STATE_READ_SENT) {
  	int8u result = buffer[2];
    int16u attributeId = (buffer[0] + (buffer[1] << 8));
                                                             ...
                                                             int16u bufLen)
{
  if (clusterId == testClusterId
  	  && readWriteState == READ_WRITE_TEST_STATE_WRITE_SENT) {
    int16u attributeId = (buffer[1] + (buffer[2] << 8));
    if (attributeId != (currentIndex + attributeIdStart)) {
      return;
    }
    emberEventControlSetInactive(emberAfPluginTestHarnessReadWriteAttributesTimeoutEventControl);
