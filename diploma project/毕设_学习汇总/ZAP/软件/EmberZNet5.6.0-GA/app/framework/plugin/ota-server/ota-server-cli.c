...
void setPolicy(void)
{
  if (emberCurrentCommand->name[0] == 'i') {
    // The cli.xml file specifies milliseconds.
    int16u periodMS = (int16u)emberUnsignedCommandArgument(0);
    // This ota-server-policy function specifies milliseconds.
    emAfOtaServerPolicySetMinBlockRequestPeriod(periodMS);
  } else {
...
void otaSendUpgradeCommand(void)
{
  ...
  MEMPGMCOPY(&id, &emberAfInvalidImageId, sizeof(EmberAfOtaImageId));
