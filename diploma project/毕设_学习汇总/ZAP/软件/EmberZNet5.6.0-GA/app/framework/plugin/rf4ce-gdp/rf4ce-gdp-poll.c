...
#if defined(EMBER_SCRIPTED_TEST)
boolean nodeIsPollingServer = TRUE;
#endif
...
void emAfRf4ceGdpPollingIncomingCommandClientCallback(boolean framePending);
void emAfPendingCommandEventHandlerPollNegotiationClient(void);
boolean emAfRf4ceGdpIncomingPushAttributesPollServerCallback(void);
boolean emAfRf4ceGdpIncomingPullAttributesPollServerCallback(void);
void emAfRf4ceGdpIncomingGenericResponsePollClientCallback(EmberAfRf4ceGdpResponseCode responseCode);
boolean emAfRf4ceGdpIncomingPullAttributesResponsePollClientCallback(void);
...
void emAfRf4ceGdpPollingNotifyBindingCompletePollClientCallback(int8u pairingIndex);
void emAfRf4ceGdpPollingNotifyBindingCompletePollServerCallback(int8u pairingIndex);
EmberStatus emAfRf4ceGdpPollInternal(int8u pairingIndex, int16u vendorId, EmberAfRf4ceGdpHeartbeatTrigger trigger);
...
void emAfRf4ceGdpPollingIncomingCommandCallback(boolean framePending)
{
#if (defined(EMBER_AF_PLUGIN_RF4CE_GDP_IS_POLL_CLIENT) || defined(EMBER_SCRIPTED_TEST))
...
boolean emAfRf4ceGdpIncomingPushAttributesPollNegotiationCallback(void)
{
#if (defined(EMBER_AF_PLUGIN_RF4CE_GDP_IS_POLL_SERVER) || defined(EMBER_SCRIPTED_TEST))
...
boolean emAfRf4ceGdpIncomingPullAttributesPollNegotiationCallback(void)
{
#if (defined(EMBER_AF_PLUGIN_RF4CE_GDP_IS_POLL_SERVER) || defined(EMBER_SCRIPTED_TEST))
...
boolean emAfRf4ceGdpIncomingPullAttributesResponsePollNegotiationCallback(void)
{
#if (defined(EMBER_AF_PLUGIN_RF4CE_GDP_IS_POLL_CLIENT) || defined(EMBER_SCRIPTED_TEST))
...
void emAfRf4ceGdpPollingNotifyBindingComplete(int8u pairingIndex)
{
#if (defined(EMBER_AF_PLUGIN_RF4CE_GDP_IS_POLL_CLIENT) || defined(EMBER_SCRIPTED_TEST))
...
EmberStatus emberAfRf4ceGdpPoll(int8u pairingIndex,
                                int16u vendorId,
...
static void finishPollNegotiationProcedure(boolean success);
static void scheduleNextTimeBasedPoll(int8u newPairingIndex, boolean init);

// Static variables.
static int8u currentPollingPairingIndex = 0xFF;
// Here we store for each pairing when the next poll will happen.
static int32u nextPollTimeMs[EMBER_RF4CE_PAIRING_TABLE_SIZE];

EmberStatus emAfRf4ceGdpPollInternal(int8u pairingIndex,
                                     int16u vendorId,
                                     EmberAfRf4ceGdpHeartbeatTrigger trigger)
{
  EmberStatus status;
  int8u pollConfiguration[APL_GDP_POLL_CONFIGURATION_SIZE];
...
static void startPollNegotiationProcedure(void)
{
  ...
  record.value = (int8u*)emAfRf4ceGdpLocalNodeAttributes.pollConstraints;
...
static int32u getPollInterval(int8u pairingIndex)
{
  int8u pollConfiguration[APL_GDP_POLL_CONFIGURATION_SIZE];
...
static void selectClientPollProcedure(int8u pairingIndex,
                                      const int8u *clientPollConstraints) {
  int8u clientPollConfiguration[APL_GDP_POLL_CONFIGURATION_SIZE];
  int16u clientTriggersCapabilities =
  ...
  if (clientPollConstraints[POLL_CONSTRAINT_RECORD_POLLING_METHOD_ID_OFFSET]
    ...
    if (clientTriggersCapabilities & EMBER_AF_RF4CE_GDP_POLLING_TRIGGER_TIME_BASED_POLLING_ENABLED) {
      int32u minClientVal =
          emberFetchLowHighInt32u((int8u*)clientPollConstraints
