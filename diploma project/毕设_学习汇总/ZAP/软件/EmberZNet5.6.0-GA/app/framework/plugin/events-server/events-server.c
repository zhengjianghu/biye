...
typedef struct {
  int8u eventsToPublishIndex;
  int8u *remainingEventData;
  int8u remainingEventDataLen;
...
typedef struct {
  ...
  int8u sequence;
  boolean publishFullInformation;
  int16u maxPayloadLength;
  int16u totalMatchingEvents;
  int16u eventsToPublishCount;
  int16u eventsToPublish[EMBER_AF_PLUGIN_EVENTS_SERVER_EVENT_LOG_SIZE];
  int8u totalCommands;
  int8u commandIndex;
...
static EmberAfEventLog *getEventLog(int8u endpoint, EmberAfEventLogId logId);
static void clearEventLog(EmberAfEventLog *eventLog);
static void printEventLog(int8u endpoint, EmberAfEventLogId logId);
static int16u findMatchingEvents(int8u endpoint,
                                 int8u logId,
                                 int16u eventId,
                                 int32u startTime,
                                 int32u endTime,
                                 int8u maxEvents,
                                 int16u eventOffset,
                                 int16u *eventToPublishCount,
                                 int16u *eventsToPublish);
static int8u getPublishEventLogTotalCommands(int8u endpoint,
                                             int16u payloadSize,
                                             boolean publishFullInformation,
                                             int16u eventToPublishCount,
                                             int16u *eventsToPublish);
static int8u fillPublishEventLogSubPayload(int8u endpoint,
                                           PublishEventLogState *state,
                                           boolean publishFullInformation,
                                           int8u *subPayload,
                                           int16u subPayloadMaxSpace,
                                           int16u *subPayloadLen,
                                           int16u eventToPublishCount,
                                           int16u *eventsToPublish,
                                           int8u *logPayloadControl);
static int8u findNextEvent(EmberAfEventLog *eventLog,
                           int8u prevIndex,
                           int16u eventId,
                           int32u startTime,
                           int32u endTime);

static void mangleCommandForGBCSNonTOMCmd(int8u * ep, 
                                          int8u * logId);
...
static boolean clusterInitialized = FALSE;
...
void emberAfEventsClusterServerTickCallback(int8u endpoint)
{
  ...
  int8u subPayload[EMBER_AF_MAXIMUM_SEND_PAYLOAD_LENGTH];
  int16u subPayloadLen = 0;
  int8u logPayloadControl;
  int16u maxSubPayloadLen = partner.maxPayloadLength - PUBLISH_EVENT_ZCL_HEADER_LENGTH - PUBLISH_EVENT_LOG_PAYLOAD_HEADER_LENGTH;
  int8u numberOfEvents = 0;
...
boolean emberAfEventsServerClearEventLog(int8u endpoint, EmberAfEventLogId logId)
{
  int8u i;
...
kickout:
  ...
  {
    char * logMsg = "Event Log entries cleared!";
    int8u logMsgLen = 26;
    int8u index;
    EmberAfEvent event;
    ...
    event.eventData[0] = logMsgLen;

    MEMCOPY(event.eventData + 1, logMsg, logMsgLen);
...
void emberAfEventsServerPrintEventLog(int8u endpoint, EmberAfEventLogId logId)
{
                                  ...
                                  const EmberAfEvent *event)
{
  ...
  int8u index;
  if (NULL != eventLog) {
    int8u nextEntry = (eventLog->nextEntry)++;
...
static EmberAfEventLog *getEventLog(int8u endpoint, EmberAfEventLogId logId)
{
  ...
  int8u ep = emberAfFindClusterServerEndpointIndex(endpoint, ZCL_EVENTS_CLUSTER_ID);
                                 ...
                                 int16u *eventsToPublish)
{
  ...
  int8u candidateEventIndex[NUM_EVENT_LOGS];
  boolean eventsFound = FALSE;
  EmberAfEvent *nextEvent, *eventCandidate;
  int8u nextEventLogIndex;
  int16u totalMatchingEvents = 0, skippedEvents = 0;
  ...
  *eventsToPublishCount = 0;
  ...
  while (eventsFound) {
    ...
    if (nextEvent != NULL) {
      ...
      // offset given.
      int8u nextEventIndex = candidateEventIndex[nextEventLogIndex];
      ...
      if (maxEvents == ZCL_EVENTS_SEARCH_CRITERIA_PUBLISH_ALL_EVENTS
        ...
        *eventsToPublish++ = ((int16u)allLogIds[nextEventLogIndex]) << 8 | (int16u)nextEventIndex;
...
static int8u getPublishEventLogTotalCommands(int8u endpoint,
                                             ...
                                             int16u *eventsToPublish)
{
  int8u totalCommands = 0;
  int8u numberOfEvents;
  PublishEventLogState state;
  int16u subPayloadLen = 0;
  int16u maxSubPayloadLen = maxPayloadLength - PUBLISH_EVENT_ZCL_HEADER_LENGTH - PUBLISH_EVENT_LOG_PAYLOAD_HEADER_LENGTH;
...
static int8u fillPublishEventLogSubPayload(int8u endpoint,
                                           ...
                                           int8u *logPayloadControl)
{
  int16u subPayloadRemainingSpace = subPayloadMaxSpace;
  EmberAfEventLog *eventLog;
  EmberAfEvent *event;
  int8u eventDataLen;
  int8u *eventData;
  int8u numberOfEvents = 0;
  boolean eventCrossesFrameBoundary = FALSE;
  ...
  *subPayloadLen = 0;
  ...
  while (subPayloadRemainingSpace > PUBLISH_EVENT_LOG_EVENT_INFO_LENGTH
    ...
    int8u logId = (eventsToPublish[state->eventsToPublishIndex] & 0xFF00) >> 8;
    int8u eventIndex = (eventsToPublish[state->eventsToPublishIndex] & 0x00FF);
  ...
  if (logPayloadControl) {
    *logPayloadControl = (numberOfEvents << 4);
    ...
    // SE1P2-29 - GBCS IRP282 wants to set the high nibble of the control
    // control to 0, as the maximum number of log entries (100) does not
    // fit in 4 nibbles.
#if defined(EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION) \
    || defined(EMBER_AF_PLUGIN_GBCS_GAS_METER)
...
static int8u findNextEvent(EmberAfEventLog *eventLog,
                           ...
                           int32u endTime)
{
  int8u i, index = ZCL_EVENTS_INVALID_INDEX;
 ...
 *
 */
static void mangleCommandForGBCSNonTOMCmd(int8u * endpoint, 
                                          int8u * logId){
#if defined(EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION)
  int8u newLogId = *logId & 0x0F;
  if (*endpoint == EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_ESI_ENDPOINT) {
