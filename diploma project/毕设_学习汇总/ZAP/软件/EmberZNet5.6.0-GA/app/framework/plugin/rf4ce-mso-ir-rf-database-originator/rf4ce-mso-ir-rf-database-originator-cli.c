...
static int8u rfPressedPayload[RF_PAYLOAD_MAX];
static int8u rfRepeatedPayload[RF_PAYLOAD_MAX];
static int8u rfReleasedPayload[RF_PAYLOAD_MAX];
static int8u irCode[IR_CODE_MAX];
...
void emberAfRf4ceMsoIrRfDatabaseOriginatorSetIrCommand(void)
{
  ...
  entry.irDescriptor.irCodeLength
    = emberCopyStringArgument(1,
                              (int8u *)entry.irDescriptor.irCode,
                              IR_CODE_MAX,
                              FALSE); // no padding
}
