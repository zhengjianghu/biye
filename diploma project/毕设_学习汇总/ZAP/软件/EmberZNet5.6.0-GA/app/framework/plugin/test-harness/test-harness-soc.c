...
static int8u pageRangeIndex;
static EmberAesMmoHashContext context;
static int8u pageWithinRangeIndex;
#endif
...
static void hashTheFirstPage(int8u* address)
{
  const int16u offset = (// Base Address Table format
                   + 4      // top of stack
                   + 4      // reset vector
                   + 4      // nmi handler
...
{
#if defined(HASH_CODE_SUPPORT)
  int8u pageBeg = halAppAddressTable.pageRanges[pageRangeIndex].begPg;
  int8u pageEnd = halAppAddressTable.pageRanges[pageRangeIndex].endPg;
  const int8u* address = (int8u*)(PAGE_TO_PROG_ADDR(pageBeg + pageWithinRangeIndex));
...
void emAfTestHarnessStartImageStampCalculation(void)
{
  ...
  hashTheFirstPage((int8u*)(PAGE_TO_PROG_ADDR(halAppAddressTable.pageRanges[0].begPg)));
  pageWithinRangeIndex++;
