...
static void repeat10(int8u pairingIndex, int16u nowMs);
static void stop(int8u index);
...
static int8u nextCheckValidation = NULL_PAIRING_INDEX;
...
static int8u nextSetGetAttribute = NULL_PAIRING_INDEX;
...
void emAfRf4ceMsoMessageSent(int8u pairingIndex,
                             EmberAfRf4ceMsoCommandCode commandCode,
                             const int8u *message,
                             int8u messageLength,
                             EmberStatus status)
...
void emAfRf4ceMsoIncomingMessage(int8u pairingIndex,
                                 EmberAfRf4ceMsoCommandCode commandCode,
                                 const int8u *message,
                                 int8u messageLength)
{
  ...
  case EMBER_AF_RF4CE_MSO_COMMAND_USER_CONTROL_RELEASED:
    {
      ...
      boolean hasRcCommandCode = (USER_CONTROL_RELEASED_1_1_LENGTH
                                  <= messageLength);
      EmberAfRf4ceMsoKeyCode rcCommandCode
        = (hasRcCommandCode
...
static void rescheduleUserControlEvent(void)
{
  ...
  for (i = 0; i < COUNTOF(records); i++) {
    if (records[i].pairingIndex != 0xFF) {
      int16u elapsedMs = elapsedTimeInt16u(records[i].timeMs, nowMs);
      int16u remainingMs = ((EMBER_AF_PLUGIN_RF4CE_MSO_KEY_REPEAT_WAIT_TIME_MS
                             <= elapsedMs)
                            ? 0
                            : (EMBER_AF_PLUGIN_RF4CE_MSO_KEY_REPEAT_WAIT_TIME_MS
                               - elapsedMs));
