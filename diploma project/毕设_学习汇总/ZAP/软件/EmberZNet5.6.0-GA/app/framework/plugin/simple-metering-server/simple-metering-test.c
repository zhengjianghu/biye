...
static int32u getMeteringConsumptionRate(int8u endpoint)
{
  int8u multiplierArray[] = {0,0,0};
  int8u divisorArray[] = {0,0,0};
...
static void setMeteringConsumptionRate(int16u rate,int8u endpoint)
{
  int8u dataType;
  EmberStatus status;
  int32u multiplier;
...
void afTestMeterSetConsumptionRate(int16u rate,int8u endpoint) {
...
int32u testMeterIntervalEndTimes[EMBER_AF_PLUGIN_SIMPLE_METERING_SERVER_TEST_METER_PROFILES];
                                 ...
                                 int8u numberOfPeriods)
{
  ...
  int32u intervalEndTime;
  int32u ct;
  int8u start, stop, dt, i, profilesReturned;
  int8u profileIntervalPeriod = PROFILE_INTERVAL_PERIOD_TIMEFRAME;
  ...
  if (status == EMBER_ZCL_STATUS_SUCCESS) {
    ...
    int32u lastValidInterval = (ct / PROFILE_INTERVAL_PERIOD_IN_SECONDS) * PROFILE_INTERVAL_PERIOD_IN_SECONDS;
    int32u lastEndTime = (lastValidInterval >= (PROFILE_INTERVAL_PERIOD_IN_SECONDS * EMBER_AF_PLUGIN_SIMPLE_METERING_SERVER_TEST_METER_PROFILES)?
...
emberAfSimpleMeteringClusterPrintln("lastValidInterval %u lastEndTime %u",lastValidInterval,lastEndTime);
    ...
    for (i = 0; i < profilesReturned; i++) {
      int32u data = (0x00 << 24) | (testMeterProfiles[start + i][0] <<16) | (testMeterProfiles[start+i][1] << 8) | (testMeterProfiles[start+i][2]);
