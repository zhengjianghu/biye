...
typedef struct {
  ...
  int8u rcCommandPayload[EMBER_AF_PLUGIN_RF4CE_MSO_MAXIMUM_RC_COMMAND_PAYLOAD_LENGTH];
  int8u rcCommandPayloadLength;
  int16u timeMs;
  int8u numberOfTransmissions;
} UserControlRecord;
...
static EmberStatus setGetAttributeRequest(int8u pairingIndex,
                                          EmberAfRf4ceMsoAttributeId attributeId,
                                          int8u index,
                                          int8u valueLen,
                                          const int8u *value);
  ...
  for (i = 0; i < COUNTOF(records); i++) {
    if (records[i].pairingIndex != 0xFF) {
      if (timeGTorEqualInt16u(nowMs, records[i].timeMs)) {
        ...
        int16u remainingMs = elapsedTimeInt16u(nowMs, records[i].timeMs);
