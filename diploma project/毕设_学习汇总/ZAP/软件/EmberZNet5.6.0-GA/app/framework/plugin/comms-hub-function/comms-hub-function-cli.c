...
static int8u messagePayload[EMBER_AF_COMMS_HUB_FUNCTION_MSG_CACHE][EMBER_AF_COMMS_HUB_FUNCTION_SEND_LENGTH];
static int8u nextMessage = 0;

// Prototypes
static void sendMessage(EmberEUI64 deviceId, int16u length, int8u *message, boolean includeHeader);
...
// plugin comms-hub-function simulate-gbz-msg <int16u:messageCode>
void emAfPluginCommsHubFunctionCliSimulateGbzMsg(void)
{
  EmberEUI64 destinationDeviceId;
  int16u messageCode = (int16u)emberUnsignedCommandArgument(1);
  ...
  if (messageCode == GCS05_MESSAGE_CODE){
    ...
    sendMessage(destinationDeviceId, sizeof(gbzCommand), gbzCommand, TRUE);
    ...
    int8u gbzCommand[] = { 0x09, 0x01, // profile id
                           ...
                           0x12, 0x00, // extended gbz command length
                           0x09, // frame control
                           0x00, // trans. seq number
...
void emAfPluginCommsHubFunctionCliSend(void)
{
  ...
  length = emberCopyStringArgument(1,
                                   ...
                                   255,
                                   FALSE);
  sendMessage(deviceId, length, message, FALSE);
}

void emAfPluginCommsHubFunctionCliTimeout(void)
{
  int32u timeout = (int32u)emberUnsignedCommandArgument(0);
...
{
  int8u i;
  int8u *data = &(messagePayload[nextMessage][0]);
  int16u headerLength = 0;
  ...
  status = emberAfPluginCommsHubFunctionSend(deviceId, headerLength + length, messagePayload[nextMessage]);
...
// plugin comms-hub-function get-tunnel-endpoint <int16u:nodeId>
void emAfPluginCommsHubFunctionCliGetTunnelEndpoint(){
  int8u  endpoint;
  int16u nodeId;
  
  nodeId = (int16u)emberUnsignedCommandArgument(0);
