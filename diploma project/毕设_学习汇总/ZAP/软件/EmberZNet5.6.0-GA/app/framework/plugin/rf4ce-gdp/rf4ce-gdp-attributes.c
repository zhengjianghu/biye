...
boolean emAfRf4ceGdpIncomingPushAttributesIdentificationCallback(void);
...
static boolean acceptIncomingAttributeCommand(void);
static const EmAfRf4ceGdpAttributeDescriptor *getAttributeDescriptor(int8u attrId);
...
#if defined(EMBER_SCRIPTED_TEST)
static boolean arrayedAttributeIsEntryIdValid(int8u attrId,
                                              int16u entryId);
#endif
...
void emAfRf4ceGdpAttributesInitCallback(void)
{
  ...
  int8u pollConstraintsDefault[APL_GDP_POLL_CONSTRAINTS_SIZE] =
...
static void handleIncomingGetOrPullAttributesCommand(boolean isGet)
{
  ...
  while(emAfRf4ceGdpFetchAttributeIdentificationRecord(&idRecord)) {
    ...
    int8u attributeVal[MAX_GDP_ATTRIBUTE_SIZE];
