...
static int8u heap[EMBER_AF_PLUGIN_RF4CE_MSO_IR_RF_DATABASE_RECIPIENT_HEAP_SIZE];
static int8u *tail = heap;


static int8u getEntryIndex(EmberAfRf4ceMsoKeyCode keyCode);
static int32u calculateHeapUsage(const EmberAfRf4ceMsoIrRfDatabaseEntry *entry);
static int8u *findHead(const EmberAfRf4ceMsoIrRfDatabaseEntry *entry);
static int8u *copyRfPayload(boolean hasDescriptor,
...
static EmberStatus setEntry(int8u index,
                                                    ...
                                                    EmberAfRf4ceMsoIrRfDatabaseEntry *entry)
{
  ...
  int8u index = getEntryIndex(keyCode);
  if (index == 0xFF) {
                                                               ...
                                                               int8u entryIndex)
{
  ...
  // entryIndex is the key code.  If we don't support the key code, we return
  // FALSE.
  int8u index = getEntryIndex((EmberAfRf4ceMsoKeyCode)entryIndex);
  if (index == 0xFF) {
    return FALSE;
  }
                            ...
                            const EmberAfRf4ceMsoIrRfDatabaseEntry *entry)
{
  ...
  // invalid entry.
  int32u reclaimableBytes = calculateHeapUsage(&database[index]);
