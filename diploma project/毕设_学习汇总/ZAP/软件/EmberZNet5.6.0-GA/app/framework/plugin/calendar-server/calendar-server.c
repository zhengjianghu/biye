...
typedef struct {
  ...
  int8u sequence;
  int8u calendarsToPublish[EMBER_AF_PLUGIN_CALENDAR_COMMON_TOTAL_CALENDARS];
...
void emberAfCalendarClusterServerTickCallback(int8u endpoint)
{
  ...
  if (publishInfo.publishCommandId == ZCL_PUBLISH_CALENDAR_COMMAND_ID) {
    int8u calendarIndex = publishInfo.calendarsToPublish[publishInfo.commandIndex];
    ...
    int8u dayIndex = publishInfo.startIndex + publishInfo.commandIndex;
    ...
    int8u weekIndex = publishInfo.startIndex + publishInfo.commandIndex;
                                                     ...
                                                     int8u numberOfDays)
{
  ...
  int8u calendarIndex = emberAfPluginCalendarCommonGetCalendarById(issuerCalendarId,
                                                                   providerId);
