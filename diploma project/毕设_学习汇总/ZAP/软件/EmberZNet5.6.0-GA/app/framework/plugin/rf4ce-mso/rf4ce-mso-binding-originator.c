...
static int8u currentCandidateCount;

// The current state of the binding process.
static int8u internalState;
...
static boolean isValidDiscoveryResponse(const EmberRf4ceApplicationInfo *appInfo);
static boolean generatePairingCandidatesList(void);
static int8u getPairingCandidatesListSize(void);
static boolean pairingCandidatesListEntryInUse(int8u index);
static int8u getHighestRankedCandidateIndex(void);
static void pairWithCandidate(int8u index);
...
{
  EmberStatus status;
  int8u profileIdList[] = { EMBER_AF_RF4CE_PROFILE_MSO };
  int8u bindPairingIndex = emAfRf4ceMsoGetActiveBindPairingIndex();
...
EmberStatus emberAfRf4ceMsoWatchdogKick(int16u validationWatchdogTimeMs)
{
  return EMBER_INVALID_CALL;
}
...
EmberStatus emberAfRf4ceMsoAbortValidation(boolean fullAbort)
{
  return EMBER_INVALID_CALL;
}
...
boolean emAfRf4ceMsoIsBlackedOut(int8u pairingIndex)
{
  ...
  if (getPairingCandidatesListSize() == 0) {
    ...
    if (generatePairingCandidatesList()) {
      ...
      if (getPairingCandidatesListSize() == 0) {
        ...
        int8u index = getHighestRankedCandidateIndex();
...
static int8u getCurrentClassDescriptor(int8u index)
{
  int8u classDescriptorInUse =
      (emAfRf4ceMsoPairingCandidatesList[index].control
...
static int8u getHighestRankedCandidateIndex(void)
{
  int8u highestRankedEntryIndex = 0xFF;
  int8u i;
...
// aborted.
static boolean handleDuplicateClassNumber(int8u index, boolean *reclassified)
{
  int8u handlingField = (getCurrentClassDescriptor(index)
                         & MSO_CLASS_DESCRIPTOR_DUPLICATE_CLASS_NUMBER_HANDLING_MASK)
                         >> MSO_CLASS_DESCRIPTOR_DUPLICATE_CLASS_NUMBER_HANDLING_OFFSET;
...
// Return TRUE if the binding procedure can continue, FALSE if it should be
// aborted.
static boolean duplicateClassNumberHandlingProcedure(void)
{
  ...
  do {
    ...
    for(i=0; i<NWK_MAX_REPORTED_NODE_DESCRIPTORS; i++) {
      ...
      for(j=i+1; j<NWK_MAX_REPORTED_NODE_DESCRIPTORS; j++) {
        int8u jClassNumber = (getCurrentClassDescriptor(j)
                              & MSO_CLASS_DESCRIPTOR_CLASS_NUMBER_MASK);
