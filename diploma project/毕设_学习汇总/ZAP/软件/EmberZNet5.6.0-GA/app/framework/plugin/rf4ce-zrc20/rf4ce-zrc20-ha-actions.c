...
void emAfRf4ceZrcHAActionsBindingCompleteCallback(int8u pairingIndex)
{
#if defined(EMBER_AF_RF4CE_ZRC_IS_HA_ACTIONS_ORIGINATOR) || defined(EMBER_SCRIPTED_TEST)
...
static int8u announcementOrPullRequestPairingIndex;
static int8u haSupportedInstanceIndex;
static int8u haAttributeIdPull;
static int8u pullRequestDirtyFlags[ZRC_BITMASK_SIZE];

static boolean isApiCallValid(int8u pairingIndex);
static void finishAnnouncement(EmberStatus status);
static boolean pushHomeAutomationSupportedToServer(void);
static void continueServingHomeAutomationPullRequest(void);

EmberStatus emberAfRf4ceZrc20StartHomeAutomationSupportedAnnouncement(int8u pairingIndex)
{
...
EmberStatus emberAfRf4ceZrc20PullHomeAutomationAttribute(int8u pairingIndex,
                                                         int16u vendorId,
                                                         int8u haInstanceId,
                                                         int8u haAttributeId)
{
...
void emAfRf4ceZrcStartHomeAutomationAnnouncementClient(int8u pairingIndex)
{
#if defined(EMBER_SCRIPTED_TEST)
...
boolean emAfRf4ceZrcIncomingPullAttributesResponseHomeAutomationOriginatorCallback(void)
{
  ...
  if ((emAfZrcState == ZRC_STATE_HA_ORIGINATOR_PULLING_HA_ATTRIBUTE_FROM_RECIPIENT
    ...
    int8u savedZrcState = emAfZrcState;
    EmberAfRf4ceZrcHomeAutomationAttribute haAttribute;
    boolean attributeContentIsValid = FALSE;
    ...
    if (record.status == EMBER_AF_RF4CE_GDP_ATTRIBUTE_STATUS_SUCCESS) {
      ...
      haAttribute.contents = (int8u*)(record.value + 1);
...
void emAfRf4ceZrcIncomingRequestHomeAutomationPull(int8u haInstanceId,
                                                   const int8u *haAttributeDirtyFlags)
{
...
static boolean pushHomeAutomationSupportedToServer(void)
{
  ...
  while(haSupportedInstanceIndex <= HA_SUPPORTED_MAX_INSTANCE_ID_INDEX
    ...
    record.value = (int8u*)haSupported.contents;
...
static void continueServingHomeAutomationPullRequest(void)
{
  ...
  for (i = 0; i <= MAX_INT8U_VALUE; i++) {
    if (emAfRf4ceZrcReadActionBank(pullRequestDirtyFlags, (int8u)i)) {
...
static boolean acceptIncomingAttributeCommand(int8u pairingIndex);
