...
#if defined(EMBER_SCRIPTED_TEST)
boolean nodeIsIdentificationServer = TRUE;
#endif
...
void emAfRf4ceGdpNotifyBindingCompleteIdentificationClientCallback(int8u pairingIndex);
void emAfRf4ceGdpNotifyBindingCompleteIdentificationServerCallback(int8u pairingIndex);
...
boolean emAfRf4ceGdpIncomingPushAttributesIdentificationServerCallback(void);
EmberStatus emAfPluginRf4ceGdpIdentifyClient(int8u pairingIndex);
...
void emAfRf4ceGdpIdentificationNotifyBindingComplete(int8u pairingIndex)
{
#if (defined(EMBER_AF_PLUGIN_RF4CE_GDP_IS_IDENTIFICATION_CLIENT) || defined(EMBER_SCRIPTED_TEST))
...
static void finishIdentificationProcedure(boolean success);
