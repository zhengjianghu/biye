...
typedef struct{
  ...
  int8u  debtType;    // Indicates if the entry is for Debt #1, #2, or #3
} emDebtLogEntry;
...
static int8u getNextAvailableDebtLogIndex( void );
static int8u getMostRecentDebtLogEntry( int8u debtType );
static int8u getNthMostRecentDebtLogEntry( int8u debtType, int8u nth, int8u mostRecentIndex );
void emberAfPluginLogDebtCollectionEvent( int32u collectionTimeUtc, int32u amountCollected, int32u outstandingDebt, int8u debtType );

static void emberAfPluginPrepaymentServerPublishDebtLog( EmberNodeId nodeId, int8u srcEndpoint, int8u dstEndpoint,
...
static int8u getNextAvailableDebtLogIndex(){
  int8u i;
  int32u minCollectionTime = 0xFFFFFFFF;
  int8u  minCollectionTimeIndex;
...
static int8u getMostRecentDebtLogEntry( int8u debtType ){
  ...
  int8u i;
  int32u minDeltaTime = 0xFFFFFFFF;
  int8u  minDeltaTimeIndex = 0xFF;
  int32u currentTime;
  int32u deltaTime;
...
void emberAfPluginUpdateDebtOnCollectionEvent( int8u endpoint, int8u debtType ){
  // int8u i;
  int16u attributeId;
  ...
  if( debtType <= DEBT_TYPE_3 ){
      ...
      if( (status == EMBER_ZCL_STATUS_UNSUPPORTED_ATTRIBUTE) ||
        ...
        if( (status == EMBER_ZCL_STATUS_SUCCESS) && (debtAmount > 0) && (debtRecoveryAmount > 0) ){
          ...
          emberAfWriteAttribute( endpoint, ZCL_PREPAYMENT_CLUSTER_ID, attributeId,  CLUSTER_MASK_SERVER,
                          (int8u *)&debtAmount, ZCL_INT32U_ATTRIBUTE_TYPE );
        }
      }
    }
    ...
    if( i < DEBT_LOG_TABLE_SIZE ){
      DebtLogTable[i].collectionTime = now;
      DebtLogTable[i].amountCollected = debtRecoveryAmount;
      DebtLogTable[i].outstandingDebt = debtAmount;
      DebtLogTable[i].debtType = debtType;
    }*/
  }
}
...
static int8u NumDebtPayloads;
// Formats the debt log response data prior to calling the function to send the log data.
void emberAfPluginSendPublishDebtLog( EmberNodeId nodeId, int8u srcEndpoint, int8u dstEndpoint, 
                                      int32u latestEndTime, int8u numberOfDebts, int8u debtType ){

  DebtPayload debtPayloads[ DEBT_LOG_TABLE_SIZE ];    // Size this to max number of log entries.
  int8u  i, x;
  int8u mostRecentIndex;
...
boolean emberAfPrepaymentClusterChangeDebtCallback(int32u issuerEventId,
  ...
  if( (debtAmount != DEBT_ATTRIB_NO_CHANGE32) && (debtAmountType & DEBT_TYPE_INCREMENT_FLAG) ){
    ...
    emberAfReadAttribute( endpoint, ZCL_PREPAYMENT_CLUSTER_ID, attributeId,
                      CLUSTER_MASK_SERVER, (int8u *)&currentDebt, 4, NULL );
  ...
  if( debtRecoveryStartTime != DEBT_ATTRIB_NO_CHANGE32 ){
    ...
    emberAfWriteAttribute( endpoint, ZCL_PREPAYMENT_CLUSTER_ID, attributeId,  CLUSTER_MASK_SERVER,
                           (int8u *)&debtRecoveryStartTime, ZCL_UTC_TIME_ATTRIBUTE_TYPE );
  }
  else{
    attributeId = ZCL_DEBT_RECOVERY_START_TIME_1_ATTRIBUTE_ID + attributeGroupOffset;
    emberAfReadAttribute( endpoint, ZCL_PREPAYMENT_CLUSTER_ID, attributeId, CLUSTER_MASK_SERVER,
                           (int8u *)&debtRecoveryStartTime, 4, NULL );
  }
  ...
  if( debtRecoveryCollectionTime != DEBT_ATTRIB_NO_CHANGE16 ){
    ...
    emberAfWriteAttribute( endpoint, ZCL_PREPAYMENT_CLUSTER_ID, attributeId,  CLUSTER_MASK_SERVER,
                           (int8u *)&debtRecoveryCollectionTime, ZCL_INT16U_ATTRIBUTE_TYPE );
  }
  else{
    attributeId = ZCL_DEBT_RECOVERY_COLLECTION_TIME_1_ATTRIBUTE_ID + attributeGroupOffset;
    emberAfReadAttribute( endpoint, ZCL_PREPAYMENT_CLUSTER_ID, attributeId, CLUSTER_MASK_SERVER,
                           (int8u *)&debtRecoveryCollectionTime, 2, NULL );
  }
  ...
  if( debtRecoveryAmount != DEBT_ATTRIB_NO_CHANGE32 ){
    ...
    emberAfWriteAttribute( endpoint, ZCL_PREPAYMENT_CLUSTER_ID, attributeId,  CLUSTER_MASK_SERVER,
                           (int8u *)&debtRecoveryAmount, ZCL_INT32U_ATTRIBUTE_TYPE );
  }
  ...
  if( debtRecoveryBalancePercentage != DEBT_ATTRIB_NO_CHANGE16 ){
    ...
    emberAfWriteAttribute( endpoint, ZCL_PREPAYMENT_CLUSTER_ID, attributeId,  CLUSTER_MASK_SERVER,
                           (int8u *)&debtRecoveryBalancePercentage, ZCL_INT16U_ATTRIBUTE_TYPE );
  }
...
void emberAfPluginPrepaymentPrintDebtLogIndex( int8u index ){
...
// Return that amount.
int32u emberAfPluginPrepaymentGetDebtRecoveryTopUpPercentage( int8u endpoint, int32u topUpValue ){
  EmberAfStatus status;
  int32u reduction;
  int16u attributeId;
  int16u percentage;
  int16u totalPercentage = 0;
  int8u  recoveryMethod;
  int8u  i;

  for( i=0; i<3; i++ ){
    attributeId = ZCL_DEBT_RECOVERY_TOP_UP_PERCENTAGE_1_ATTRIBUTE_ID + (i * DEBT_RECOVERY_GROUP_DELTA);
    status =  emberAfReadAttribute( endpoint, ZCL_PREPAYMENT_CLUSTER_ID, attributeId, CLUSTER_MASK_SERVER,
                (int8u *)&percentage, 2, NULL );

    attributeId = ZCL_DEBT_RECOVERY_METHOD_1_ATTRIBUTE_ID + (i * DEBT_RECOVERY_GROUP_DELTA);
    status |= emberAfReadAttribute( endpoint, ZCL_PREPAYMENT_CLUSTER_ID, attributeId, CLUSTER_MASK_SERVER,
                (int8u *)&recoveryMethod, 1, NULL );
...
void emberAfPluginPrepaymentPrintDebtAttributes( int8u endpoint, int8u index ){
  int16u attributeId;
  EmberAfStatus status;
  int32u val32u;
  int16u val16u;
