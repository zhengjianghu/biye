...
static int8u actionRemapStatusTable[EMBER_AF_PLUGIN_RF4CE_ZRC20_ACTION_MAPPING_SERVER_ACTION_REMAP_TABLE_SIZE/8+1];
#ifdef EMBER_AF_PLUGIN_RF4CE_GDP_IS_POLL_SERVER
static int8u actionRemapNegotiateRequestTable[EMBER_RF4CE_PAIRING_TABLE_SIZE/8+1];
...
static int8u actionMappingsServerHeap[EMBER_AF_PLUGIN_RF4CE_ZRC20_ACTION_MAPPING_SERVER_ACTION_MAPPINGS_HEAP_SIZE];
static int8u *tail = actionMappingsServerHeap;

static void removeOldEntry(int16u index);
static int32u calculateHeapUsage(const EmberAfRf4ceZrcActionMapping *entry);
static int8u *findHead(const EmberAfRf4ceZrcActionMapping *entry);
static int8u *copyActionData(boolean hasDescriptor,
                             const EmberAfRf4ceZrcActionMapping *entry);
static int8u *copyIrCode(boolean hasDescriptor,
                         const EmberAfRf4ceZrcActionMapping *entry);

static void setMappingDirty(int16u serverIndex);
static void clearMappingDirty(int8u pairingIndex, int16u clientIndex);
static void requestSelectiveAMUpdatePerPairing(int8u pairingIndex);
#ifdef EMBER_AF_PLUGIN_RF4CE_GDP_IS_POLL_SERVER
static void heartbeatCallback(int8u pairingIndex,
                                                            ...
                                                            EmberAfRf4ceZrcActionMapping* actionMapping)
{
  ...
  for (i=0; i<EMBER_AF_PLUGIN_RF4CE_ZRC20_ACTION_MAPPING_SERVER_MAPPABLE_ACTIONS_TABLE_SIZE; i++) {
    ...
    if (0x00 == MEMCOMPARE(mappableAction,
      ...
      setMappingDirty(i);
#ifdef EMBER_AF_PLUGIN_RF4CE_GDP_IS_POLL_SERVER
      int8u status = emAfRf4ceGdpGetPairingBindStatus(actionRemapTable[i].pairingIndex);
      ...
      boolean pollClient = READBITS(status, PAIRING_ENTRY_POLLING_ACTIVE_BIT);
      if (pollClient) {
        return EMBER_SUCCESS;
      }
#endif // EMBER_AF_PLUGIN_RF4CE_GDP_IS_POLL_SERVER
  ...
  for (i=0; i<EMBER_AF_PLUGIN_RF4CE_ZRC20_ACTION_MAPPING_SERVER_MAPPABLE_ACTIONS_TABLE_SIZE; i++) {
    ...
    if (tmpMappableAction.actionDeviceType == EMBER_AF_RF4CE_DEVICE_TYPE_RESERVED) {
      ...
      for (j=0; j<EMBER_RF4CE_PAIRING_TABLE_SIZE; j++) {
        ...
        if (emberAfRf4cePairingTableEntryIsActive(&entry)) {
#ifdef EMBER_AF_PLUGIN_RF4CE_GDP_IS_POLL_SERVER
          int8u st = emAfRf4ceGdpGetPairingBindStatus(j);
          // If we are a poll server and the client is also a poll client, we have to
          // wait for a heartbeat before we can send the notification command.
          // Otherwise, we can just send the message right away.
          boolean pollClient = READBITS(st, PAIRING_ENTRY_POLLING_ACTIVE_BIT);
...
int16u emberAfRf4ceZrc20ActionMappingServerGetMappableActionCount(void)
{
...
void emAfRf4ceZrc20ActionMappingServerClearMapping(int16u serverIndex)
{
  int16u i;
...
EmberStatus emAfRf4ceZrc20ActionMappingServerUpdateOrAddMapping(int8u pairingIndex,
                                                                int16u entryIndex,
                                                                int16u serverIndex)
{
  int16u i;
...
EmberStatus emAfRf4ceZrc20ActionMappingServerRemoveMapping(int8u pairingIndex,
                                                           int16u entryIndex)
{
  int16u i;
...
EmberStatus emAfRf4ceZrc20ActionMappingServerGetMapping(int8u pairingIndex,
                                                        int16u entryIndex,
                                                        int16u *serverIndex)
{
  int16u i;
...
EmberStatus emAfRf4ceZrc20ActionMappingServerClearMappableAction(int16u index)
{
  // Index out of range.
...
EmberStatus emAfRf4ceZrc20ActionMappingServerSetMappableAction(int16u index,
                                                               EmberAfRf4ceZrcMappableAction *mappableAction)
{
  // Index out of range.
  if (index >= EMBER_AF_PLUGIN_RF4CE_ZRC20_ACTION_MAPPING_SERVER_MAPPABLE_ACTIONS_TABLE_SIZE) {
...
EmberStatus emAfRf4ceZrc20ActionMappingServerGetMappableAction(int16u index,
                                                               EmberAfRf4ceZrcMappableAction *mappableAction)
{
  // Index out of range.
  if (index >= EMBER_AF_PLUGIN_RF4CE_ZRC20_ACTION_MAPPING_SERVER_MAPPABLE_ACTIONS_TABLE_SIZE) {
...
EmberStatus emAfRf4ceZrc20ActionMappingServerClearActionMapping(int16u index)
{
...
{
  MEMSET(actionMappingsServerTable, 0, sizeof(actionMappingsServerTable));
  MEMSET(actionMappingsServerHeap, 0, sizeof(actionMappingsServerHeap));
}

EmberStatus emAfRf4ceZrc20ActionMappingServerSetActionMapping(int16u index,
                                                              EmberAfRf4ceZrcActionMapping *actionMapping)
{
  // Index out of range.
  if (index >= EMBER_AF_PLUGIN_RF4CE_ZRC20_ACTION_MAPPING_SERVER_MAPPABLE_ACTIONS_TABLE_SIZE) {
...
EmberStatus emAfRf4ceZrc20ActionMappingServerGetActionMapping(int16u index,
                                                              EmberAfRf4ceZrcActionMapping *actionMapping)
{
  // Index out of range.
  if (index >= EMBER_AF_PLUGIN_RF4CE_ZRC20_ACTION_MAPPING_SERVER_MAPPABLE_ACTIONS_TABLE_SIZE) {
...
static void setMappingDirty(int16u serverIndex)
{
  ...
  for (i=0; i<EMBER_AF_PLUGIN_RF4CE_ZRC20_ACTION_MAPPING_SERVER_ACTION_REMAP_TABLE_SIZE; i++) {
    if (actionRemapTable[i].serverIndex == serverIndex) {
        SETBIT(actionRemapStatusTable[i/8], (int8u)i & 0x07);
      }
  }
}
...
static void clearMappingDirty(int8u pairingIndex, int16u clientIndex)
{
  ...
  for (i=0; i<EMBER_AF_PLUGIN_RF4CE_ZRC20_ACTION_MAPPING_SERVER_ACTION_REMAP_TABLE_SIZE; i++) {
    if (actionRemapTable[i].pairingIndex == pairingIndex
      ...
      CLEARBIT(actionRemapStatusTable[i/8], (int8u)i & 0x07);
      return;
    }
  }
}
...
static void requestSelectiveAMUpdatePerPairing(int8u pairingIndex)
{
  int16u i;
  // TODO: use defines!
  int8u payload[CLIENT_NOTIFICATION_REQUEST_SELECTIVE_AM_UPDATE_INDEX_LIST_LENGTH_LENGTH+256*2];
  int8u indexListLen = 0;
  boolean actionMappingChanged = FALSE;
  ...
  for (i=0; i<EMBER_AF_PLUGIN_RF4CE_ZRC20_ACTION_MAPPING_SERVER_ACTION_REMAP_TABLE_SIZE; i++) {
    if (actionRemapTable[i].pairingIndex == pairingIndex
        && READBIT(actionRemapStatusTable[i/8], (int8u)i & 0x07)) {
      *(int16u*)(&payload[indexListLen*2 + CLIENT_NOTIFICATION_REQUEST_SELECTIVE_AM_UPDATE_INDEX_LIST_OFFSET])
...
static void heartbeatCallback(int8u pairingIndex,
                              EmberAfRf4ceGdpHeartbeatTrigger trigger)
{
  int8u status = emAfRf4ceGdpGetPairingBindStatus(pairingIndex);
  // If we are a poll server and the client is also a poll client, we have to
  // wait for a heartbeat before we can send the notification command.
  // Otherwise, we can just send the message right away.
  boolean pollClient = READBITS(status, PAIRING_ENTRY_POLLING_ACTIVE_BIT);
