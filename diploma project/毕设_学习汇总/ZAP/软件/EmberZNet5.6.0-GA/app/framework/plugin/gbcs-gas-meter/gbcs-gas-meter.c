...
static int8u message[TEMP_MESSAGE_LEN];
...
static int8u mirrorEndpoint = 0xFF;
static int16u mirrorAddress = 0;
...
static EmberAfAttributeId meteringClusterAttributes[] = {
    ...
    ZCL_ATTRIBUTE_REPORTING_STATUS_ATTRIBUTE_ID
};
...
static int8u data[GAS_METER_ATTRIBUTE_RECORD_BUFFER_SIZE];
static int8u clusterToReport = 0;
static int8u attributeToReport = 0;

//------------------------------------------------------------------------------
// Forward Declarations

static void setSleepyMeterState(int8u newState);
static void serviceDiscoveryCallback(const EmberAfServiceDiscoveryResult* result);
static boolean tunnelCreate(EmberNodeId remoteNodeId,
                            int8u remoteEndpoint);
static boolean tunnelDestroy(void);
static boolean tunnelSendData(int16u dataLen,
                              int8u *data);
static boolean requestTunnel(void);
static boolean handleRequestTunnelFailure(EmberAfPluginTunnelingClientStatus status);
static void handleRemotePartyMessage(int16u gbzCommandLen, int8u *gbzCommand);
...
{
  int8u i;
  int8u reportingStatus = EMBER_ZCL_ATTRIBUTE_REPORTING_STATUS_ATTRIBUTE_REPORTING_COMPLETE;
...
void emberAfClusterInitCallback(int8u endpointId,
                         int16u clusterId) {
  int8u deviceType = EMBER_ZCL_METERING_DEVICE_TYPE_GAS_METERING;
...
void emberAfPluginGbcsGasMeterSleepyMeterEventHandler(void)
{
  ...
  case MIRROR_UPDATE_IN_PROGRESS :
    ...
    if (clusterToReport < GAS_METER_REPORT_LIST_COUNT) {
      ...
      int8u reportingStatus = EMBER_ZCL_ATTRIBUTE_REPORTING_STATUS_PENDING;
  ...
  if (state == MIRROR_READY || state == MIRROR_UPDATE_IN_PROGRESS) {
    if (attributeId == ZCL_NOTIFICATION_FLAGS_4_ATTRIBUTE_ID) {
      ...
      if (bitMap & EMBER_AF_METERING_FNF_TUNNEL_MESSAGE_PENDING) {
        ...
        int8u header = TUNNEL_MANAGER_HEADER_GET;
...
boolean emberAfSimpleMeteringClusterRequestMirrorResponseCallback(int16u endpointId)
{
  if (endpointId == 0xffff) {
    ...
    if (state != MIRROR_READY) {
      ...
      int32u issuerEventId = emberAfGetCurrentTime();
...
static void handleRemotePartyMessage(int16u gbzCommandLen, int8u *gbzCommand)
{
  ...
  boolean validGbzMessage;
  int16u headerLen = 0;
  int16u gbzResponseLen = 0;
