...
static boolean sendMirrorReportAttributeResponse(int8u endpoint, int8u index);
...
void emClearMirrorByEndpoint(int8u endpoint)
{
...
boolean emberAfPluginMeterMirrorGetEndpointByEui64(EmberEUI64 eui64,
                                                   int8u *returnEndpoint)
{
  int8u index = findMirrorIndex(eui64);
  if (index == INVALID_INDEX) {
    return FALSE;
  }
...
int16u emberAfPluginMeterMirrorRequestMirror(EmberEUI64 requestingDeviceIeeeAddress)
{
  int8u meteringDeviceTypeAttribute =  EMBER_ZCL_METERING_DEVICE_TYPE_UNDEFINED_MIRROR_METER;
...
int16u emberAfPluginMeterMirrorRemoveMirror(EmberEUI64 requestingDeviceIeeeAddress)
{
  int8u index;
  int8u endpoint;
                                        ...
                                        int16u bufLen)
{
  ...
  int8u endpoint;
  int8u index;
  boolean attributeReportingComplete = FALSE;
...
{
  int8u status;
  int8u notificationFlagsBuffer[20];
  int8u flagsBufferLen = 0;
  int16u attrId;
  int32u readFlag;
  ...
  for (attrId = ZCL_NOTIFICATION_FLAGS_2_ATTRIBUTE_ID;
    ...
    status = emberAfReadClientAttribute(endpoint,
                                        ...
                                        sizeof(int32u));
    if (status != EMBER_ZCL_STATUS_SUCCESS) {
      MEMSET(&notificationFlagsBuffer[flagsBufferLen], 0, sizeof(int32u));
    } else {
      emberAfCopyInt32u(notificationFlagsBuffer, flagsBufferLen, readFlag);
    }
    flagsBufferLen += 4;
