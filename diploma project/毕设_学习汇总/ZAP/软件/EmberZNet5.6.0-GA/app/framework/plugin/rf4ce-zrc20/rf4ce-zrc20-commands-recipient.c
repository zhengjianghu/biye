...
// The scheduling code below depends on the wait time fitting in an int16u.
...
static boolean repeat(EmberAfRf4ceZrcActionRecord *record);
...
static void sow(int8u pairingIndex);
static void reap(int8u pairingIndex);
  ...
  case EMBER_AF_RF4CE_ZRC_COMMAND_ACTIONS:
    {
      ...
      for (i = 0; i < 2; i++) {
        ...
        int8u offset = ZRC_PAYLOAD_OFFSET;
        ...
        while (offset + 1 + 1 + 1 + 1 <= messageLength) {
          EmberAfRf4ceZrcActionRecord record;
          int8u actionControl = message[offset++];
          record.pairingIndex = pairingIndex;
          ...
          if (EMBER_AF_RF4CE_ZRC_ACTION_BANK_VENDOR_SPECIFIC_EXPLICIT
            ...
            if (offset + 2 <= messageLength) {
              record.actionVendorId = emberFetchLowHighInt16u((int8u *)(message + offset));
              offset += 2;
            } else {
...
static void reschedule(void)
{
  ...
  for (i = 0; i < COUNTOF(records); i++) {
    if (records[i].pairingIndex != 0xFF) {
      ...
      int16u remainingMs = ((EMBER_AF_PLUGIN_RF4CE_ZRC20_ACTION_REPEAT_WAIT_TIME_MS
                             <= elapsedMs)
                            ? 0
                            : (EMBER_AF_PLUGIN_RF4CE_ZRC20_ACTION_REPEAT_WAIT_TIME_MS
                               - elapsedMs));
