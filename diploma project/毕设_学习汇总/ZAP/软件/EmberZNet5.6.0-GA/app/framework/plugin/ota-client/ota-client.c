...
// HA 1.2 feature
static int16u minBlockRequestPeriodMs;
...
void emberAfPluginOtaClientStackStatusCallback(EmberStatus status)
{
  ...
  // the setTimer(int32u) function wants milliseconds
...
#if defined(ZCL_USING_OTA_BOOTLOAD_CLUSTER_MINIMUM_BLOCK_REQUEST_PERIOD_ATTRIBUTE)
        emberAfPutInt16uInResp(minBlockRequestPeriodMs);
#endif
      }
      break;
...
static EmberAfStatus imageBlockResponseParse(int8u* buffer, int8u index, int8u length)
{
  ...
  int32u currentOffset;
  int8u dataSize;
  int32u timerMs = (EMBER_AF_PLUGIN_OTA_CLIENT_DOWNLOAD_DELAY_MS >= minBlockRequestPeriodMs
  ...
  if (buffer != NULL && status == EMBER_ZCL_STATUS_WAIT_FOR_DATA) {
    ...
    if (length >= IMAGE_BLOCK_RESPONSE_WAIT_FOR_DATA_MIN_BLOCK_RESPONSE_LENGTH) {
      int16u temp = emberAfGetInt16u(buffer, 
                                     ...
                                     length);
      emberAfCoreFlush();
      otaPrintln("Updating Min Block Request Period to %d milliseconds",
                 temp);
