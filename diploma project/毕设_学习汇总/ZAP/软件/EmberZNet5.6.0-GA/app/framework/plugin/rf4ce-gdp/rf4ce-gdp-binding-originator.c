...
static int8u savedProfileIdList[7];
static int8u savedProfileIdListLength;
static int8u savedSearchDevType;
static EmberRf4cePairingTableEntry savedPairingEntry;
static int8u savedPairingEntryIndex;

static boolean bindProcessAborted;
...
static EmberStatus startDiscoveryProcess(int8u* profileIdList,
...
static boolean checkBindingProcedureTimer(void);

static boolean maybeRestoreSavedPairingEntry(void);
...
static boolean comparePairingCandidates(EmAfGdpPairingCanditate *pc1,
                                        EmAfGdpPairingCanditate *pc2,
                                        boolean *abort,
                                        boolean *lqiTieBreaker);
...
static int8u getLowestOrHighestRankedCandidateIndex(boolean highest,
                                                    boolean freshOnly,
                                                    boolean *abort);
...
EmberStatus emberAfRf4ceGdpBind(int8u *profileIdList,
                                int8u profileIdListLength,
                                int8u searchDevType)
{
  EmberStatus status;
                                                              ...
                                                              int8u discRequestLqi)
{
  ...
  int8u matchingProfileIdList[EMBER_RF4CE_APPLICATION_PROFILE_ID_LIST_MAX_LENGTH];
  int8u matchingProfileIdListLength;
  int8u sameRecipientEntryIndex = 0xFF;
  int8u freeEntryIndex = 0xFF;
  int8u minLqiThreshold;
  int8u gdpVersion;
  int8u i;
  ...
  if (sameRecipientEntryIndex == 0xFF) {
    ...
    if (freeEntryIndex == 0xFF) {
      ...
      // ranks higher.
      int8u lowestCandidateIndex =
          getLowestOrHighestRankedCandidateIndex(LOWEST,
                                                 ALL_CANDIDATES,
                                                 &bindProcessAborted);

      if (!bindProcessAborted) {
        boolean newCandidateRanksHigher =
                                                ...
                                                const EmberRf4ceApplicationInfo *appInfo)
{
  ...
  if (status == EMBER_SUCCESS || status == EMBER_DUPLICATE_ENTRY) {
    int8u gdpVersion = emAfRf4ceGdpGetGdpVersion(appInfo->profileIdList,
                                                 emberAfRf4ceProfileIdListLength(appInfo->capabilities));
    ...
    if (gdpVersion == GDP_VERSION_2_0) {
      int8u bindStatus = (emAfRf4ceGdpGetPairingBindStatus(pairingIndex)
...
boolean emAfRf4ceGdpIncomingGetAttributesResponseOriginatorCallback(void)
{
...
boolean emAfRf4ceGdpIncomingPullAttributesResponseOriginatorCallback(void)
{
  EmberAfRf4ceGdpAttributeStatusRecord statusRecord;
...
void emAfRf4ceGdpIncomingCheckValidationResponseOriginator(EmberAfRf4ceGdpCheckValidationStatus status)
{
  ...
  if (emAfTemporaryPairingIndex == emberAfRf4ceGetPairingIndex()
    ...
    int8u bindStatus = ((PAIRING_ENTRY_BINDING_STATUS_BOUND_ORIGINATOR
                         << PAIRING_ENTRY_BINDING_STATUS_OFFSET)
                        | PAIRING_ENTRY_BINDING_COMPLETE_BIT
...
static EmberStatus setDiscoveryRequestAppInfo(void)
{
  ...
  int8u appUserString[USER_STRING_APPLICATION_SPECIFIC_USER_STRING_LENGTH] =
      EMBER_AF_PLUGIN_RF4CE_GDP_APPLICATION_SPECIFIC_USER_STRING;
  int8u *finger = appInfo.userString;
...
static EmberStatus setPairRequestAppInfo(int8u *targetProfileIdList,
                                         int8u targetProfileIdListLength,
                                         int8u gdpVersion,
                                         boolean isProxy)
{
  EmberRf4ceApplicationInfo appInfo;
  ...
  if (isProxy) {
    ...
    int8u matchingProfileIdListLength =
        emAfCheckDeviceTypeAndProfileIdMatch(0xFF, // we don't bother matching the device type here.
                                             (int8u*)emAfRf4ceApplicationInfo.deviceTypeList,
...
static boolean maybeRestoreSavedPairingEntry(void)
{
  if (savedPairingEntryIndex < 0xFF) {
    int8u bindStatus = (emAfRf4ceGdpGetPairingBindStatus(savedPairingEntryIndex)
                        | PAIRING_ENTRY_BINDING_COMPLETE_BIT);
...
static void startGdpConfigurationProcedureOrFail(void)
{
  ...
  int8u version[APL_GDP_VERSION_SIZE];
  int8u capabilities[APL_GDP_CAPABILITIES_SIZE];
  ...
  attributes[0].value = (int8u*)version;
  ...
  attributes[1].value = (int8u*)capabilities;
...
static boolean firstTimerCheck;
static void startBindingProcedureTimer(void)
{
...
static boolean comparePairingCandidates(EmAfGdpPairingCanditate *pc1,
                                        ...
                                        boolean *lqiTieBreaker)
{
  int8u pc1CurrentClassDescriptor = pc1->primaryClassDescriptor;
  int8u pc2CurrentClassDescriptor = pc2->primaryClassDescriptor;
  int8u pc1ClassInUse = PRIMARY_CLASS;
  int8u pc2ClassInUse = PRIMARY_CLASS;
  int8u pc1ClassNumber;
  int8u pc2ClassNumber;
  int8u i;
  boolean dummy;
...
{
  int8u i;
  int8u lowestOrHighestCandidateIndex = 0xFF;
  ...
  for(i=0; i<APL_MAX_PAIRING_CANDIDATES;i++) {
    if (isPairingCandidateEntryInUse(i)
      ...
      if (lowestOrHighestCandidateIndex == 0xFF) {
        ...
        boolean currentCandidateRanksHigher =
