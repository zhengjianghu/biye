...
void emberAfPluginDrlcServerScheduleMandatoryLce(void){
  ...
  event.eventId                 = (int32u)emberUnsignedCommandArgument(2);
  event.deviceClass             = (int16u)emberUnsignedCommandArgument(3);
  event.utilityEnrollmentGroup  = (int8u) emberUnsignedCommandArgument(4);
  event.startTime               = (int32u)emberUnsignedCommandArgument(5);
  event.duration                = (int16u)emberUnsignedCommandArgument(6);
  event.criticalityLevel        = (int8u) emberUnsignedCommandArgument(7);
  event.eventControl            = (int8u) emberUnsignedCommandArgument(8);
  // Optionals
