...
{
  emberAfPutInt8uInResp(value->year);
  emberAfPutInt8uInResp(value->month);
  emberAfPutInt8uInResp(value->dayOfMonth);
  emberAfPutInt8uInResp(value->dayOfWeek);
}
...
int8u emberAfGetDate(int8u* message, int16u currentIndex, int16u msgLen, EmberAfDate *destination)
{
