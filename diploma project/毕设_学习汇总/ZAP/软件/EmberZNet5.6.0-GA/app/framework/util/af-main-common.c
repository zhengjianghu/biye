                       ...
                       int16u length)
{
  ...
  if (apsFrame->clusterId == SIMPLE_DESCRIPTOR_RESPONSE) {
    ...
    emberAfZdoPrintln(", Matches: 0x%2x",                       (int16u)(message[1])
                      + (int16u)(message[2] << 8));
  }
...
EmberStatus emAfFormNetwork(EmberNetworkParameters *parameters,
                            boolean centralizedNetwork)
{
  EmberStatus status = EMBER_INVALID_CALL;
#ifdef EMBER_AF_HAS_COORDINATOR_NETWORK
  if (emAfProIsCurrentNetwork()
      && emAfCurrentZigbeeProNetwork->nodeType == EMBER_COORDINATOR) {
    zaTrustCenterSecurityInit(centralizedNetwork);
...
{
  return emAfFormNetwork(parameters,
                         TRUE); // Yes, centralized network.
}
