...
#define ACTIVE_ENDPOINT_RESPONSE_NODE_ID_OFFSET (EMBER_AF_ZDO_RESPONSE_OVERHEAD + 1)
#define ACTIVE_ENDPOINT_RESPONSE_LIST_OFFSET (ACTIVE_ENDPOINT_RESPONSE_NODE_ID_OFFSET + 2)
                                               ...
                                               int16u length) 
{
  ...
  // and we cannot just cast the message pointer into an int16u* array.
  // However we also do it for the little endian CPUs because the 
  // message can actually be truncated due to bad Zigbee stacks.
  int16u clusters[MAX_CLUSTERS_IN_ACTIVE_ENDPOINT_RESPONSE];
  int8u index = SIMPLE_DESCRIPTOR_RESPONSE_INPUT_CLUSTER_LIST_INDEX;
...
static boolean processActiveEndpointResponse(State* state,
                                             const int8u* message,
                                             int16u length)
{
  EmberAfServiceDiscoveryResult result;
  ...
  int8u count = message[ACTIVE_ENDPOINT_RESPONSE_COUNT_OFFSET];

  // +1 for count value iteslf, +2 for node ID
  int8u expectedLength = EMBER_AF_ZDO_RESPONSE_OVERHEAD + 1 + 2 + count;  
