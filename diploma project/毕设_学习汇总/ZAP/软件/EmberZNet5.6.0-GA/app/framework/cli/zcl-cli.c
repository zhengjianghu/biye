...
#if defined(EMBER_AF_PLUGIN_ZIGBEE_INTERNET_CLIENT)
#include "app/framework/plugin/zigbee-internet-device-common/zigbee-internet-device-common.h"
#include "app/framework/plugin/zigbee-internet-client/zigbee-internet-client.h"
#endif
...
int8u disableDefaultResponse = 0;
...
void emAfApsFrameEndpointSetup(int8u srcEndpoint,
                                      int8u dstEndpoint) {
...
void emAfApsFrameClusterIdSetup(int16u clusterId)
{
...
void zclBufferAddBuffer(const int8u *buffer, int8u length)
{
...
void zclBufferAddByteFromArgument(int8u index)
{
  zclBufferAddByte((int8u)emberUnsignedCommandArgument(index));
}

void zclBufferAddInt8sFromArgument(int8u index)
{
  zclBufferAddByte((int8u)((int8s)emberSignedCommandArgument(index)));
}

void zclBufferAddWordFromArgument(int8u index)
{
  zclBufferAddWord((int16u)emberUnsignedCommandArgument(index));
}

void zclBufferAddInt32FromArgument(int8u index)
{
...
// into the zcl buffer.
void zclBufferAddLengthAndStringFromArgument(int8u argIndex)
{
  // We protect against copying outside of the appZclBuffer
  // by passing the maximum length of the buffer, or a maximum
  // value of an int8u.
  int16u maxLength = APP_ZCL_BUFFER_SIZE - (appZclBufferLen + 1);
...
void zclSimpleCommand(int8u frameControl,
                             ...
                             int8u commandId)
{
  int8u argumentIndex, typeIndex;
  int8u count = emberCommandArgumentCount();
  int8u type;
...
void zclGlobalSetup(int8u commandId)
{
...
{
  disableDefaultResponse = (int8u)emberUnsignedCommandArgument(0);
...
boolean addScenesOnOffValue;
#endif
#ifdef ZCL_USING_LEVEL_CONTROL_CLUSTER_CLIENT
int8u addScenesLevelValue;
#endif
...
{
  int8u i;
  int8u numScheduledPhases = (int8u)emberUnsignedCommandArgument(1);
...
//   int8u destEndpoint = (int8u)emberUnsignedCommandArgument(2);
