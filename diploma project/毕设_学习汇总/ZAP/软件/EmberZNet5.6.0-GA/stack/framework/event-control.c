...
static boolean allowIdling = FALSE;
...
//#define DEBUG_EVENTS
  ...
  static void emTrackNextEvent(EmberEventControl *event)
  {
    ...
    int32u eventTime = event->timeToExecute;
...
void emEventControlSetDelayMS(EmberEventControl*event, int32u delay)
{
...
int32u emEventControlGetRemainingMS(EmberEventControl *event)
{
  ...
  static int32u emTaskMsUntilRun(EmberTaskControl *task, int32u now)
  {
...
/* Not currently used
int32u emberTaskMsUntilRun(EmberTaskId taskid)
{
  ...
  for (nextEvent = events; ; nextEvent++) {
    ...
    if (control->status != EMBER_EVENT_INACTIVE) {
      if (control->status == EMBER_EVENT_ZERO_DELAY
        ...
        int32u duration = elapsedTimeInt32u(nowMS32, control->timeToExecute);
        if (duration < maxMs) {
          maxMs = duration;
          if (returnIndex != NULL) {
            *returnIndex = index;
...
EmberTaskId emberTaskInit(EmberEventData *events)
{
  ...
  boolean emberMarkTaskIdle(EmberTaskId taskid)
  {
    ...
    EmberTaskId id;
    int32u now;
    int32u msToNextEvent = MAX_INT32U_VALUE;
