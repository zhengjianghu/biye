...
int32u *emChildTimers = (int32u *) &emAvailableMemory[END_emChildStatus];
#define END_emChildTimers               \
 (END_emChildStatus + align(EMBER_CHILD_TABLE_SIZE * sizeof(int32u)))
...
int8u *emRouteRecordTable = (int8u *) &emAvailableMemory[END_emRouteData];
...
int16u *emBindingRemoteNode = (int16u *) &emAvailableMemory[END_emRf4cePendingOutgoingPacketTable];
int8u emberBindingTableSize = EMBER_BINDING_TABLE_SIZE;
#define END_emBindingRemoteNode        \
 (END_emRf4cePendingOutgoingPacketTable + align(EMBER_BINDING_TABLE_SIZE * sizeof(int16u)))
...
int8u *emEndDeviceTimeout = &emAvailableMemory[END_emBindingFlags];
#define END_emEndDeviceTimeout \
 (END_emBindingFlags + align(EMBER_CHILD_TABLE_SIZE * sizeof(int8u)))
...
int8u emEndDeviceConfiguration = EMBER_END_DEVICE_CONFIG_PERSIST_DATA_ON_PARENT;
#endif
...
/* Sample system call */
int32u emberTest (unsigned int arg, ... )
{
  return arg;
}
