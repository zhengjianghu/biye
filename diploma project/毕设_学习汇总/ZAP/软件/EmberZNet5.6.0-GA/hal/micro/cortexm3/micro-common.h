...
/**
 * @brief Read the voltage level on an IO pin
 *
 * @param gpio   The IO pin to use, can be specified with the convenience macros
 *               PORTA_PIN(), PORTB_PIN(), PORTC_PIN(), etc.
...
typedef int8u SysClkCallbackContext;
 ...
 */
int16u halSystemClockEnableCallback(SysClkCallbackContext context);
 ...
 */
int32u halPeripheralClockHz(void);
...
#define MAC_TIMER_TICKS(microseconds) \
  ((((int32u)(microseconds) * (halPeripheralClockHz() / 250000)) + 47) / 48)
...
#define MAC_TIMER_US(ticks) \
  (((((int32u)(ticks) * 48000) / (halPeripheralClockHz() / 250000)) + 500) / 1000)
...
#define SYSTICK_TIMER_TICKS(microseconds) \
  ((((int32u)(microseconds) * (halMcuClockHz() / 250000)) + 3) / 4)
...
#define SYSTICK_TIMER_US(ticks) \
  (((((int32u)(ticks) * 8) / (halMcuClockHz() / 250000)) + 1) / 2)
 ...
 */
void halInternalScheduleSysTickInMicroseconds(int32s usDelay);
 ...
 */
EmberStatus halInternalUartSetBaudRate(int8u port, int32u baud);
 ...
 */
int32u halInternalUartGetBaudRate(int8u port);
 ...
 * in process.
 */
int16u halCommonTryToSwitchToXtal(SysClkCallbackContext context);
 ...
 */
EmberStatus halSleepForQsWithOptions(int32u *duration, WakeMask gpioWakeBitMask);
 ...
 */
void halInternalContextSaveRestore(int32u save);
