...
#elif EM_SERIAL2_ENABLED && !EM_SERIAL1_ENABLED
  // Only Serial 2 is enabled
...
#elif EM_SERIAL2_ENABLED && EM_SERIAL1_ENABLED
  // Both Serial 1 and 2 are enabled
...
void usbReleaseBuffer( USB_Status_TypeDef status, int32u xferred, int32u remaining )
{
    ...
    if (EM_SER3_PORT_EN(port)) {
        ...
        usbTxData();
          ...
          while ( q->used > 0 ) {
            ...
            if (!USBD_EpIsBusy(EP_IN))
            {
              USBD_Write( EP_IN, q->nextByte, (int32u)q->lastByte - (int32u)q->nextByte + 1, (USB_XferCompleteCb_TypeDef) &usbReleaseBuffer );
...
void halInternalUartRxIsr(int8u port, int16u causes)
{
    ...
    if (EM_SER_MULTI(EM_SER1_PORT_FIFO(port) || EM_SER2_PORT_FIFO(port))) {
      ...
      while ( SCx_REG(port, UARTSTAT) & SC_UARTRXVAL ) {
        ...
        static boolean uartFixTried = FALSE;
