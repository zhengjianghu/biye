...
void halInternalSleep(SleepModes sleepMode)
{
  ...
  WakeMask gpioWakeSel  = (GPIO_PAWAKE<<0);
           ...
           gpioWakeSel |= ((int64u)GPIO_PEWAKE<<32);
#endif
#ifdef EMBER_WAKE_PORT_F
           gpioWakeSel |= ((int64u)GPIO_PFWAKE<<40);
#endif
...
deepSleepCore:
      ...
      //stick all our saved info onto stack since it's only temporary
      {
        ...
        boolean skipSleep = FALSE;
          ...
          int32u ramRetainValue = (1<<((RAM_SIZE_B - UNRETAINED_RAM_SIZE)/
          ...
          if((ramRetainValue>>(RETAIN_BITS/2)) == 0) {
            ...
            int8u *ptr = (int8u*)(DATA_BIG_INFO_BASE | MFG_PART_DATA_LOCATION);
            if(ptr[2] >= 0x01) {
        ...
        int64u powerUpEvents = PWRUP_EVENT;
        ...
        if((!skipSleep) &&
          ...
          if((INT_CFGSET&INT_SLEEPTMR) && (INT_PENDSET&INT_SLEEPTMR)) {
            ...
            int32u ticks = MAC_TIMER_TICKS(1000); // 1 millisecond
            int32u beginTime;
            forceSleepTmrCnt = FALSE;
...
void halSleep(SleepModes sleepMode)
{
  ...
  WakeMask gpioWakeBitMask  = (EMBER_WAKE_PORT_A << 0) |
                            ...
                            #endif
                            #ifdef EMBER_WAKE_PORT_E
                              |( (int64u) EMBER_WAKE_PORT_E << 32)
                            #endif
                            #ifdef EMBER_WAKE_PORT_F
                              |( (int64u) EMBER_WAKE_PORT_F << 40)
                            #endif
                              ;
