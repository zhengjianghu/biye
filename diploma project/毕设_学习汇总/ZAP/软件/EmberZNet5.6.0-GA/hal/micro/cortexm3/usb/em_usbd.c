...
int USBD_Init( const USBD_Init_TypeDef *p )
{
  ...
  int32u totalRxFifoSize, totalTxFifoSize, numInEps, numOutEps;
...
int USBD_RemoteWakeup( void )
{
  ...
  int16u timeout = 4; // Set for 4 ms. Host should take over within 1 ms
  ...
  int16u curTime = halCommonGetInt16uMillisecondTick();
...
int8u dequeueTxIntoBuffer(int8u *data)
{
...
void usbTxData ()
{
  ...
  {
    int8u data[EP5_SIZE];
    int8u txSize = dequeueTxIntoBuffer(data);
    if (txSize>0)
    {
      USBD_Write( EP_IN, data, txSize, NULL );
    }
  }
}
...
{
  while (USBD_EpIsBusy(EP_IN)) {}
  USBD_Write( EP_IN, data, length, NULL );
  while (USBD_EpIsBusy(EP_IN)) {}
}
