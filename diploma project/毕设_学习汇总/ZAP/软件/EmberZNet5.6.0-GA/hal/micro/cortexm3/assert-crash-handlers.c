...
#else  // RTOS
  #define freeRTOS 0
  // extern int32u* xTaskGetCurrentTaskStackTop(void);
  // extern int32u* xTaskGetCurrentTaskStackBottom(void); 
#endif // RTOS
...
{
  for ( ; p < (int32u *)STACK_SEGMENT_END; p++) {
...
int16u halInternalCrashHandler(void)
{
  ...
  int8u i, j;
  int32u *sp, *s, *sEnd, *stackBottom, *stackTop; 
  int32u data;
  ...
  if(freeRTOS && (c->LR & 4)) {
    ...
    // stackBottom = (int32u*)xTaskGetCurrentTaskStackBottom();
    // stackTop = (int32u*)xTaskGetCurrentTaskStackTop();
    stackBottom = sp;
    stackTop = sp + 8;
    c->processSPUsed = stackTop - sp;
  } else {    
    stackBottom = (int32u*)c->mainStackBottom;
    stackTop = (int32u*)STACK_SEGMENT_END;
  ...
  int32u safeStackBottom = c->mainStackBottom;
  if (safeStackBottom < (int32u) RESETINFO_SEGMENT_END) {
    safeStackBottom = (int32u) RESETINFO_SEGMENT_END;
  }
  c->mainSPUsed = halInternalGetMainStackBytesUsed((int32u*)safeStackBottom);
