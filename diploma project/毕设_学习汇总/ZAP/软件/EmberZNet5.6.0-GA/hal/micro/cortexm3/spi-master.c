...
#include "app/util/serial/serial.h"     // To assess SERIAL1 & SERIAL3 modes
...
SPIDBG(extern int8u serialPort;)
...
#define EVAL3(a,b,c)       PASTE(a,b,c)
  ...
  #define INT_SCxCFG(port)              (*((volatile int32u*)(intScxCfg[port])))
  #define INT_SCxFLAG(port)             (*((volatile int32u*)(intScxFlag[port])))


  //== GLOBALS ==

  static const int32u scxBaseAddr[] = {
    0,                  // SC0 "bitbang"
  ...
  static const int32u intScxFlag[] = {
    0,                  // SC0 "bitbang"
 ...
 #if     ENABLE_SPI_INTERNAL_ISRS
  static const int32u scxVectorIndex[] = {
    0,                  // SC0 "bitbang"
  ...
  static void scxIsrHandler(EmberSpiPort port)
  {
    ...
    int16u rxLen = SCx_REG(port, RXCNTA); // Use RXCNTA for how much received
    int32u rxDma = (rxLen > 0) ? SCx_REG(port, RXBEGA) : 0;
    ...
    (*(SCx_CB(port)))(port, SCx_REG(port, TXCNT), (int8u*) rxDma);
  }
...
#define GPIO_Px_OFFSET        (GPIO_PBCFGL_ADDR - GPIO_PACFGL_ADDR)
  ...
  static int8u  spiBbCfg;
  static int16u spiBbCycleNs;

  static void   spiBbDelayNs(int16u ns)
  {
    //FIXME: Just run as fast as we can in bitbang mode, ignoring actual speed
  }
#endif//SC0_SCLK

static void halInternalGpioClr(int8u gpio)
{
  *((volatile int32u *)(GPIO_PxCLR_BASE+(GPIO_Px_OFFSET*(gpio/8)))) = BIT(gpio&7);
}

static void halInternalGpioSet(int8u gpio)
{
...
static void halInternalGpioCfg(int8u gpio, int32u cfg)
{
  volatile int32u* reg = (GPIO_PxCFG_BASE+(GPIO_Px_OFFSET*(gpio/8))+((gpio/4)&1));
  int8u shift = 4*(gpio%4);
 ...
 */
static int8u halSpiMasterTransferByte(EmberSpiPort port, int8u txByte)
{
  //assert(port < EMBER_SPI_PORT_MAX);
 #ifdef  SC0_SCLK
  if (port == EMBER_SPI_PORT_BITBANG) {
    int8u bits = 8;
    int8u bitmask = (spiBbCfg & SC_SPIORD) ? 0x01 : 0x80;
                             ...
                             boolean lsbFirst)
{
  ...
  if (port == EMBER_SPI_PORT_BITBANG) {
    ...
    resetSpi(port);
   #if     ENABLE_SPI_INTERNAL_ISRS
    halRegisterRamVector(SCx_VECTOR_INDEX(port), (int32u) SCx_ISR(port));
   #endif//ENABLE_SPI_INTERNAL_ISRS
 ...
 */
boolean halSpiMasterIsBusy(EmberSpiPort port)
{
  boolean isBusy = FALSE; // Assume not busy, always the case for BitBang
  //assert(port < EMBER_SPI_PORT_MAX);
 #if    (ENABLE_SPI_SC > 0)
  if (port != EMBER_SPI_PORT_BITBANG) {
 ...
 */
int8u halSpiMasterDmaThreshold = SPI_USE_DMA_THRESHOLD;
                                    ...
                                    const EmberSpiTransferCallback opDoneCB)
{
  ...
  int8u spLen = (opFlags & EMBER_SPI_OP_SPLIT_LEN);
  assert( (spLen == 0)
  ...
  if ( (trLen < halSpiMasterDmaThreshold)   // Small transfer
     ...
     ||(((int32u) txBuf) < RAM_BOTTOM)      // txBuf is outside
     ||(((int32u) txBuf) > RAM_TOP)         //   of RAM bounds
     ||( (rxBuf != NULL)                    // rxBuf exists and
       &&( (((int32u) rxBuf) < RAM_BOTTOM)  // rxBuf is outside
         ||(((int32u) rxBuf) > RAM_TOP) ) ) //   of RAM bounds
     ) {
    ...
    if (spLen > 0) {
      ...
      SCx_REG(port, TXBEGA)  = (int32u)  txBuf;
      SCx_REG(port, RXBEGA)  = (int32u)  txBuf;
      SCx_REG(port, TXENDA)  = (int32u) (txBuf + spLen - 1); // END is inclusive
      SCx_REG(port, RXENDA)  = (int32u) (txBuf + spLen - 1); // END is inclusive
      SCx_REG(port, TXBEGB)  = (int32u)  rxBuf;
      SCx_REG(port, RXBEGB)  = (int32u)  rxBuf;
      SCx_REG(port, TXENDB)  = (int32u) (rxBuf + trLen - spLen - 1); // END is inclusive
      SCx_REG(port, RXENDB)  = (int32u) (rxBuf + trLen - spLen - 1); // END is inclusive
      ...
      SCx_REG(port, TXBEGA) = (int32u)  txBuf;
      SCx_REG(port, TXENDA) = (int32u) (txBuf + trLen - 1); // END is inclusive
      if (rxBuf == NULL) { // Tx only
        ...
        SCx_REG(port, RXBEGA)  = (int32u)  rxBuf;
        SCx_REG(port, RXENDA)  = (int32u) (rxBuf + trLen - 1); // END is inclusive
