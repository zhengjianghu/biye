...
#define EP_DATA_OUT       0x05  /* Endpoint for USB data reception.       */
#define EP_DATA_IN        0x85  /* Endpoint for USB data transmission.    */
...
#define USB_TX_BUF_SIZ   EP1_SIZE    /* Packet size when transmitting on USB.  */
...
STATIC_CONST_STRING_DESC( iManufacturer, L"Silicon Laboratories Inc." );
STATIC_CONST_STRING_DESC( iProduct     , L"Silicon Labs CDC Serial Port" );

#define nibble2Ascii(n) ((n) + (((n) < 10) ? '0'  : 'A' - 10));
#define LENGTH_SER_NUM_STRING_DESCRIPTOR  (2+((8*2)*4))
