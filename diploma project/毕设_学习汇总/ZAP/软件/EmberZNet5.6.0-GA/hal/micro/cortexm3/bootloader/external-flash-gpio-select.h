...
// Abstract away registers to make access serial controller agnostic
  ...
  // Default to PB7 when board header doesn't fully define alternate
  #if    !defined(CONFIGURE_EEPROM_POWER_PIN)   \
      || !defined(SET_EEPROM_POWER_PIN)         \
      || !defined(CLR_EEPROM_POWER_PIN)
    ...
    #elif    defined(CONFIGURE_EEPROM_POWER_PIN)  \
          || defined(SET_EEPROM_POWER_PIN)        \
          || defined(CLR_EEPROM_POWER_PIN)
    ...
    #define CONFIGURE_EEPROM_POWER_PIN()  do { halGpioSetConfig(PORTB_PIN(7), GPIOCFG_OUT); } while (0)
    #define SET_EEPROM_POWER_PIN()        do { GPIO_PBSET = PB7; } while (0)
    #define CLR_EEPROM_POWER_PIN()        do { GPIO_PBCLR = PB7; } while (0)
