...
#include "hal/micro/sim-eeprom.h"
  ...
  #else //EMBER_TEST
    VAR_AT_SEGMENT(NO_STRIPPING int8u simulatedEepromStorage[SIMEE_SIZE_B], __SIMEE__);
  #endif //EMBER_TEST
  ...
  const int16u VIRTUAL_PAGE_SIZE = VP_SIZE;
  const int16u REAL_PAGE_SIZE = (FLASH_PAGE_SIZE_HW);
  const int8u REAL_PAGES_PER_VIRTUAL = ((SIMEE_SIZE_HW/FLASH_PAGE_SIZE_HW)/ \
                                         NUMBER_OF_VIRTUAL_PAGES);
  const int16u VPA_BASE = (SIMEE_BASE_ADDR_HW);
  const int16u VPA_TOP = (SIMEE_BASE_ADDR_HW +
                          VP_SIZE - 1);
  const int16u VPB_BASE = (SIMEE_BASE_ADDR_HW +
                           VP_SIZE);
  const int16u VPB_TOP = (SIMEE_BASE_ADDR_HW +
                          VP_SIZE +
                          VP_SIZE - 1);
  const int16u VPC_BASE = (SIMEE_BASE_ADDR_HW +
                           VP_SIZE +
                           VP_SIZE);
  const int16u VPC_TOP = (SIMEE_BASE_ADDR_HW +
                          VP_SIZE +
                          VP_SIZE +
                          VP_SIZE - 1);
#endif //CORTEXM3
...
#define TOKEN_DEF(name,creator,iscnt,isidx,type,arraysize,...) \
  ...
  int16u ptrCache[ 0 //Compiler error here means too many elements in an array token
    #include "stack/config/token-stack.h"
  ];
#undef TOKEN_DEF
...
const int32u totalTokenStorage[(0 //Compiler error here means total token storage exceeds limit
    + (TOKEN_COUNT*2) 
    #include "stack/config/token-stack.h"
...
int8u lookupCache[ 0 //Compiler error here means too many tokens declared
...
#define TOKEN_DEF(name,creator,iscnt,isidx,type,arraysize,...) \
  ...
  const int8u sizeCache[ 0 //Compiler error here means a token's size is too large or a counter token is wrong size
    #include "stack/config/token-stack.h"
  ] = {
#undef TOKEN_DEF
#define TOKEN_DEF(name,creator,iscnt,isidx,type,arraysize,...) \
