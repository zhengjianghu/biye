...
#define TOKEN_DEF(name,creator,iscnt,isidx,type,arraysize,...) \
  ...
  const int8u sizeCache[ 0 //Compiler error here means a token's size is too large
    #include "stack/config/token-stack.h"
  ] = {
#undef TOKEN_DEF
#define TOKEN_DEF(name,creator,iscnt,isidx,type,arraysize,...) \
...
boolean checkForSimEe2DataExistence(int32u simEe2offset);
